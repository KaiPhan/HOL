...
Theorem Lyapunov_ineq_lemma:
    !m u p. measure_space m /\
            ...
            1 < p /\ p < PosInf /\
            u IN lp_space p m  ==>
            pos_fn_integral m (λx. abs (u x)) <= seminorm p m u * ((measure m (m_space m)) powr (1 - inv(p)))
Proof
    rpt STRIP_TAC
 ...
 >- (rw [seminorm_def] \\
    ...
    ‘1 = Normal 1’ by rw [] >> simp [] \\
     Know ‘pos_fn_integral m (λx. Normal 1) =  measure m (m_space m)’
     >- (MP_TAC (Q.SPECL [‘m’, ‘1’] pos_fn_integral_const) \\
         impl_tac >> simp [] \\
...
Proof
 ...
 >- (rw [seminorm_def] \\
     ...
     POP_ORW \\
     Know ‘0 <= pos_fn_integral m (λx. abs (u x) powr r')’
     >- (MATCH_MP_TAC pos_fn_integral_pos >> simp[] >> METIS_TAC [abs_pos, powr_pos]) >> DISCH_TAC \\
     ‘pos_fn_integral m (λx. abs (u x) powr r') powr (r'⁻¹ * r) = (pos_fn_integral m (λx. abs (u x) powr r') powr r'⁻¹) powr r’
         by rw [GSYM powr_powr])
 >> DISCH_TAC >> FULL_SIMP_TAC std_ss []
 >> Q.ABBREV_TAC ‘A =  pos_fn_integral m (λx. abs (u x) powr r)’
 ...
 >> ‘pos_fn_integral m (λx. abs (u x) powr r) powr inv(r) = seminorm r m u’ by rw [seminorm_def]
 >> FULL_SIMP_TAC std_ss []
...
Theorem lim_null :
  !f l.
    (?N. !n. N <= n ==> f n <> PosInf /\ f n <> NegInf) /\ l <> PosInf /\ l <> NegInf ==>
...
Theorem IN_PSPACE_PROD_SIGMA :
    !a b z. z IN p_space (a × b) <=> FST z IN p_space a /\ SND z IN p_space b
Proof
...
Theorem real_random_variable_fst :
    !p q X.
      ...
      real_random_variable (X o FST) (p × q)
Proof
    rpt STRIP_TAC
...
Theorem real_random_variable_snd :
    !p q X.
      ...
      real_random_variable (X o SND) (p × q)
Proof
    rpt STRIP_TAC
 >> Know ‘(p_space (p × q),events (p × q)) =
          (p_space p,events p) × (p_space q,events q)’
...
Theorem IN_MEASURABLE_BOREL_SUM_CMUL :
    !a f g s z.
      FINITE s /\ sigma_algebra a /\ (!i. i IN s ==> f i IN Borel_measurable a) /\
      (!x. x IN space a ==> g x = Normal z * ∑ (λi. f i x) s) ==>
      g IN Borel_measurable a
Proof
    RW_TAC std_ss []
 >> Cases_on `Normal z = 0`
 >- METIS_TAC [IN_MEASURABLE_BOREL_CONST, mul_lzero]
 >> Q.ABBREV_TAC ‘h = λx. ∑ (λi. (f: 'b -> 'a -> extreal) i x) s’
...
Proof
 ...
 >> ‘!x. x IN space a ==> f x pow n <> NegInf /\ f x pow n <> PosInf’ by rw [pow_not_infty]
 >> simp []
...
Theorem sup_not_in_imp_le:
    !s (a:extreal). (!(x:extreal). a <= x ==> x NOTIN s) /\ a NOTIN s ==> sup s <= a
Proof
  rpt STRIP_TAC
  >> Know ‘!x. x IN s ==> x < a’
  >- (GEN_TAC \\
...
Definition CinftyR_def :
    CinftyR = { f | (!n x. higher_differentiable n f x) /\
                           (!n. bounded (IMAGE (diffn n f) univ(:real))) }
End

Definition C_b_def :
    C_b <=> { f | f continuous_on univ(:real) /\ bounded (IMAGE f univ(:real)) }
End

Theorem CinftyR_subset_C3 :
    CinftyR SUBSET CnR 3
Proof
...
Proof
  ...
  >- (rename1 ‘!x. higher_differentiable 1 f x’ \\
      MP_TAC (Q.SPECL [‘f’, ‘univ(:real)’] derivativeTheory.DIFFERENTIABLE_IMP_CONTINUOUS_ON) \\
      rw [] \\
      Suff ‘f differentiable_on univ(:real)’
      >- (simp []) \\
...
Definition C_bounded_lipschitz_def :
    C_bounded_lipschitz <=> { f | Lipschitz_fun f /\ bounded (IMAGE f univ(:real)) }
End

Theorem class_lipschitz_subset_C_b :
    C_bounded_lipschitz SUBSET C_b
Proof
...
Theorem higher_differentiable_bounded_imp_lipschitz :
    !f. (!x. higher_differentiable 1 f x) /\
        bounded (IMAGE (diffn 1 f) univ(:real)) ==>
        Lipschitz_fun f
Proof
    rpt STRIP_TAC
...
Theorem C_infty_subset_class_lipschitz :
    CinftyR SUBSET C_bounded_lipschitz
Proof
...
Theorem integrable_bounded_continuous :
    !p X f. prob_space p /\
            real_random_variable X p /\
            f IN C_b ==>
            integrable p (Normal o f o real o X)
Proof
    rw [C_b_def, bounded_def]
 >> MP_TAC (Q.SPECL [‘p’, ‘λx. Normal a’, ‘Normal o f o real o X’] integrable_bounded)
 >> fs [prob_space_def]
 >> impl_tac
 >- (CONJ_TAC
...
Theorem real_random_variable_CnR_comp :
    !p f X.
      ...
      real_random_variable X p /\
      f IN CnR 3 ==>
      real_random_variable (Normal o f o real o X) p
Proof
...
Theorem add_not_infty_alt[local]:
    !a b. a + b <> PosInf /\ a <> NegInf /\ b <> NegInf ==> a <> PosInf /\ b <> PosInf
Proof
  RW_TAC std_ss []
...
Theorem integrable_alt_def :
    !m f. measure_space m ==> (integrable m f <=>
                               f IN Borel_measurable (measurable_space m) /\ ∫ m (abs o f) <> PosInf)
Proof
    rw [integrable_def]
 >> ‘∫ m (abs o f) = pos_fn_integral m (abs o f)’ by METIS_TAC [integral_abs_pos_fn]
 >> simp [FN_ABS']
 ...
 >> EQ_TAC >> simp []
 >- (STRIP_TAC \\
    Know ‘pos_fn_integral m (λx. f^+ x + f⁻ x) = pos_fn_integral m f^+ + pos_fn_integral m f⁻’
 ...
 >> art []
 >> ‘0 <= pos_fn_integral m f^+’ by METIS_TAC [pos_fn_integral_pos]
 >> ‘0 <= pos_fn_integral m f⁻’ by METIS_TAC [pos_fn_integral_pos]
...
Proof
 ...
 >- (MATCH_MP_TAC MEASURABLE_COMP \\
     ...
     METIS_TAC [p_space_def, events_def, prob_space_def, MEASURE_SPACE_SIGMA_ALGEBRA])
 >> DISCH_TAC
 >> Q.ABBREV_TAC ‘h = abs o f o FST’
 >> MP_TAC (Q.SPECL [‘m_space p’, ‘m_space q’, ‘measurable_sets p’, ‘measurable_sets q’,
                     ‘measure p’, ‘measure q’, ‘h’] (cj 5 TONELLI))
 >> simp []
 >> Know ‘h IN Borel_measurable (measurable_space p × measurable_space q)’
 >- (rw [Abbr ‘h’] \\
     irule IN_MEASURABLE_BOREL_ABS' \\
     METIS_TAC [Abbr ‘r’, MEASURE_SPACE_SIGMA_ALGEBRA])
 >> Rewr
 >> ‘(!s. FST s IN m_space p /\ SND s IN m_space q ==> 0 <= h s)’ by (rw [Abbr ‘h’, IN_PSPACE_PROD_SIGMA])
 >> simp []
 >> DISCH_TAC
 >> Know ‘∫ r h = pos_fn_integral r h’ >- (irule integral_pos_fn >> simp [Abbr ‘h’])
 >> DISCH_THEN (fs o wrap)
 >> rw [Abbr ‘h’]
 >> Know ‘0 <= pos_fn_integral p (λx. abs (f x))’ >- (irule pos_fn_integral_pos >> simp [])
 >> DISCH_TAC
 >> ‘pos_fn_integral p (λx. abs (f x)) <> NegInf’ by METIS_TAC [extreal_0_simps, lt_le]
 >> Know ‘∫ p (abs o f) = pos_fn_integral p (abs o f)’ >- (irule integral_pos_fn >> rw [])
 >> DISCH_TAC
 >> ‘pos_fn_integral p (abs o f) = pos_fn_integral p (λx. abs (f x))’ by METIS_TAC []
 >> ‘?a. pos_fn_integral p (λx. abs (f x)) = Normal a’ by METIS_TAC [extreal_cases]
 >> gs []
...
Proof
 ...
 >> DISCH_THEN (fs o wrap)
 >> rw [Abbr ‘h’]
 >> Know ‘0 <= pos_fn_integral q (λy. abs (f y))’ >- (irule pos_fn_integral_pos >> simp [])
 >> DISCH_TAC
 >> ‘pos_fn_integral q (λy. abs (f y)) <> NegInf’ by METIS_TAC [extreal_0_simps, lt_le]
 >> Know ‘∫ q (abs o f) = pos_fn_integral q (abs o f)’ >- (irule integral_pos_fn >> rw [])
 >> DISCH_TAC
 >> ‘pos_fn_integral q (abs o f) = pos_fn_integral q (λy. abs (f y))’ by METIS_TAC []
 >> ‘?a. pos_fn_integral q (λy. abs (f y)) = Normal a’ by METIS_TAC [extreal_cases]
 >> gs []
 >> fs [pos_fn_integral_const, extreal_1_simps]
QED
...
Theorem expectation_fst :
    !p q f.
      ...
      integrable p f ==>
      expectation p f = expectation (p CROSS q) (f o FST)
Proof
    rpt STRIP_TAC
 >> Q.ABBREV_TAC ‘r = p CROSS q’
 >> ‘integrable (p × q) (f o FST)’ by METIS_TAC [integrable_fst]
 ...
 >> rw []
 >> gs [integrable_alt_def]
 >> Know ‘f o FST IN Borel_measurable (measurable_space p × measurable_space q)’
 >- (MATCH_MP_TAC MEASURABLE_COMP \\
     qexists ‘(measurable_space p)’ \\
     art [] \\
     irule MEASURABLE_FST \\
 ...
 >> DISCH_TAC
 >> Know ‘pos_fn_integral (p × q) (abs o f o FST) <> PosInf’
 >- (Q.ABBREV_TAC ‘h = f o FST’ \\
     ‘h IN Borel_measurable (measurable_space r)’ by fs [] \\
     Know ‘measure_space r’
     >- (rw [Abbr ‘r’] \\
         irule measure_space_prod_measure \\
         fs [sigma_finite_measure_space_def] \\
...
Theorem expectation_snd :
    !p q f.
      ...
      integrable q f ==>
      expectation q f = expectation (p CROSS q) (f o SND)
Proof
    rpt STRIP_TAC
 >> Q.ABBREV_TAC ‘r = p CROSS q’
 >> ‘integrable (p × q) (f o SND)’ by METIS_TAC [integrable_snd]
 >> fs [prob_space_def, expectation_def]
 >> Know ‘measurable_space (p × q) = measurable_space p × measurable_space q’
 >- (rw [Abbr ‘r’] \\
     irule MEASURABLE_SPACE_PROD \\
 ...
 >> rw []
 >> gs [integrable_alt_def]
 >> Know ‘f o SND IN Borel_measurable (measurable_space p × measurable_space q)’
 >- (MATCH_MP_TAC MEASURABLE_COMP \\
     qexists ‘(measurable_space q)’ \\
     art [] \\
     irule MEASURABLE_SND \\
     METIS_TAC [MEASURE_SPACE_SIGMA_ALGEBRA])
 >> DISCH_TAC
 >> Know ‘pos_fn_integral (p × q) (abs o f o SND) <> PosInf’
...
Proof
  rw []
  >> Q.ABBREV_TAC ‘Z = λx j. if x IN p_space p then
                               ∑ (λi. Y i x) (count j) + ∑ (λi. X i x) (count n DIFF count1 j)
                             else 0’ >> gs []
  >- (gs [] \\
      ...
      Know ‘real_random_variable (λx. ∑ (λi. Y i x) (count j) + ∑ (λi. X i x) (count n DIFF count1 j)) p <=>
            real_random_variable (λx. if x IN p_space p then  ∑ (λi. Y i x) (count j) +
  ...
  >> Know ‘integrable p (λx. ∑ (λi. Y i x) (count j) + ∑ (λi. X i x) (count n DIFF count1 j)) <=>
           integrable p (λx. if x IN m_space p then  ∑ (λi. Y i x) (count j) +
                                                    ∑ (λi. X i x) (count n DIFF count1 j)
                             else 0)’
...
Proof
 ...
 >- (clt_sum2_tactic1 \\
     ...
     Suff ‘real_random_variable (λx. if x IN p_space p then
                                       ∑ (λi. Y' i x) (count j) + ∑ (λi. X' i x) (count n DIFF count1 j)
                                     else 0) p <=>
             real_random_variable (λx. if x IN p_space p then
                                         ∑ (λi. Y i x) (count j) + ∑ (λi. X i x) (count n DIFF count1 j)
                                       else 0) p’ >- (fs []) \\
 ...
 >> rw [Abbr ‘Z’, FUN_EQ_THM]
 >> POP_ASSUM (STRIP_ASSUME_TAC o Q.SPEC ‘j’) >> gs []
 >> Suff ‘integrable p (λx. if x IN p_space p then
                              ∑ (λi. Y' i x) (count j) + ∑ (λi. X' i x) (count n DIFF count1 j)
                            else 0) <=>
            integrable p (λx. if x IN p_space p then
                                ∑ (λi. Y i x) (count j) + ∑ (λi. X i x) (count n DIFF count1 j)
                              else 0)’ >- (fs [])
...
Theorem random_variable_borel_imp_Borel :
    !p X.
      ...
      random_variable (real o X) p borel ==>
...
Proof
 ...
 >> Know ‘!x. x IN m_space p ==> (Normal o real o X) x = X x’
 >- (rw [] \\
     Q.PAT_X_ASSUM ‘!x. x IN m_space p ==> _’ (STRIP_ASSUME_TAC o Q.SPEC ‘x’) \\
     fs [normal_real])
 >> DISCH_TAC
 >> MP_TAC (Q.SPECL [‘(measurable_space p)’, ‘Normal o real o X’, ‘X’] IN_MEASURABLE_BOREL_CONG')
 >> fs []
QED
...
Theorem in_measurable_borel_imp_Borel :
  !p X.
    prob_space p /\
    (!x. x IN p_space p ==> X x <> PosInf /\ X x <> NegInf) /\
    real o X IN borel_measurable (measurable_space p) ==>
    X IN Borel_measurable (measurable_space p)
Proof
  rpt STRIP_TAC
  >> MP_TAC (Q.SPECL [‘(measurable_space p)’, ‘real o X’] IN_MEASURABLE_BOREL_IMP_BOREL')
...
Theorem clt_real_random_variable_sum_compose[local] :
    !p X s f n. prob_space p /\
              ...
              0 < s n /\ s n <> PosInf /\ s n <> NegInf /\
              f IN CnR 3 ==>
              real_random_variable (Normal o f o real o ((λx. ∑ (λi. X i x) (count n) / s n))) p
Proof
    rpt STRIP_TAC
 ...
 >> ‘f IN borel_measurable borel’ by METIS_TAC [in_borel_measurable_CnR]
 >> MP_TAC (Q.SPECL [‘(p_space p,events p)’, ‘f’, ‘real o R’, ‘h’] in_measurable_borel_comp_borel)
 >> simp [] >> STRIP_TAC >> dxrule_all_then assume_tac MEASURABLE_COMP
 >> MP_TAC (Q.SPECL [‘f o real o R’, ‘p’] IN_MEASURABLE_BOREL_IMP_BOREL)
...
Theorem clt_expectation_not_infty[local] :
    !p X f. prob_space p /\
            ...
            integrable p X /\
            f IN CnR 3 ==>
            expectation p (Normal o f o real o X) <> PosInf /\
            expectation p (Normal o f o real o X) <> NegInf
Proof
    NTAC 4 STRIP_TAC
...
Theorem clt_expectation_sum_not_infty[local] :
    !p X s R f n.
      ...
      f IN CnR 3 ==>
        expectation p (Normal o f o real o R n) <> PosInf /\
        expectation p (Normal o f o real o R n) <> NegInf
Proof
    NTAC 7 STRIP_TAC
 >> MATCH_MP_TAC expectation_finite >> simp []
 >> irule integrable_bounded_continuous  >> simp []
...
Theorem in_borel_measurable_std_normal_density :
   (λx. std_normal_density x) IN borel_measurable borel
Proof
...
Proof
 ...
 >> impl_tac >- (rw [AE_T, GSYM o_DEF] >> MATCH_MP_TAC IN_MEASURABLE_BOREL_IMP_BOREL \\
                 simp [lborel_def])
 >> STRIP_TAC >> gs []
 >> Know ‘{x | x IN m_space lborel /\ f x <> 0} IN subsets borel’
    >- (MP_TAC (Q.SPECL [‘(f :real -> real)’, ‘borel’]
        ...
        Suff ‘{x | x IN m_space lborel /\ f x <> 0} = {x | f x <> 0} INTER space borel’ >> rw [] \\
        rw [EXTENSION, space_lborel, space_borel])
 >> DISCH_TAC
 >> Suff ‘0 < lambda {x | x IN m_space lborel /\ f x <> 0}’ >- (fs [lt_imp_ne])
 >> MP_TAC (Q.SPECL [‘{x |  x IN m_space lborel /\ (f :real -> real) x <> 0}’] (GSYM lebesgue_eq_lambda))
 >> rw []
 >> ‘{x | x IN m_space lborel /\ f x <> 0} = {x | f x <> 0}’ by rw [Once EXTENSION, space_lborel] >> gs []
 >> POP_ASSUM K_TAC
...
Proof
 ...
 >> Suff ‘∑ (λi. central_moment p (X i) 2) (count i) <> PosInf’
 >- (METIS_TAC [lt_infty])
...
Proof
  ...
  >- (‘Y j x =  ∑ (λi. Y i x) {j}’ by rw[REAL_SUM_IMAGE_SING] \\
      ...
      STRIP_TAC \\
      ‘{j} UNION (count j) = count (j + 1)’ by rw [Once EXTENSION] \\
      FULL_SIMP_TAC std_ss [])
  >> DISCH_THEN (fs o wrap)
  ...
  >- (‘X (j + 1) x =  ∑ (λi. X i x) {j + 1}’ by rw[REAL_SUM_IMAGE_SING] \\
      ...
      Know ‘{j + 1} UNION (count n DIFF count (j + 2)) = (count n DIFF count (j + 1))’
...
Proof
 ...
 >> ‘count1 n = count n UNION {n}’ by rw [Once EXTENSION]
 >> POP_ORW
 >> Know ‘∑ (λj. f j - f (j + 1)) (count n UNION {n}) =
 ...
 >- (Q.ABBREV_TAC ‘h = λj. f j - f (j + 1)’ \\
     ...
     Know ‘!x. x < n \/ x = n ==> h x <> NegInf’
     >- (fs [Abbr ‘h’] \\
         Know ‘!x. x < n \/ x = n ==> (λj. f j) x <> NegInf’
         >- (BETA_TAC >> rw [GSYM le_lt]) >> DISCH_TAC \\
         Know ‘!x. x < n \/ x = n ==> (λj. f (j + 1)) x <> NegInf’
         >- (BETA_TAC >> rw [GSYM le_lt]) >> DISCH_TAC \\
         fs [sub_not_infty]) >> DISCH_TAC \\
     Know ‘!x. x < n \/ x = n ==> h x <> PosInf’
     >- (fs [Abbr ‘h’] \\
         Know ‘!x. x < n \/ x = n ==> (λj. f j) x <> PosInf’
         >- (BETA_TAC >> rw [GSYM le_lt]) >> DISCH_TAC \\
         Know ‘!x. x < n \/ x = n ==> (λj. f (j + 1)) x <> PosInf’
         >- (BETA_TAC >> rw [GSYM le_lt]) >> DISCH_TAC \\
         fs [sub_not_infty]) >> DISCH_TAC \\
     ‘∑ h (count n UNION {n}) = ∑ h (count n) + h n’ by METIS_TAC [] >>  POP_ORW \\
...
Theorem SUM_SUB_GEN_WEAKEN :
    !f n.
      (!j. j <= n ==> f j <> PosInf /\ f j <> NegInf) ==>
      ∑ (λj. f j - f (j + 1)) (count n) = f 0 - f n
Proof
    rpt STRIP_TAC
 >> Q.ABBREV_TAC ‘g = λi. if i <= n then f i else 0’
 >> Know ‘!i. g i <> PosInf /\ g i <> NegInf’
 >- (GEN_TAC \\
...
Theorem clt_expectation_sum_not_infty_normal_rv[local] :
    !p N f. prob_space p /\
            ...
            f IN CnR 3 ==>
            expectation p (Normal o f o real o N) <> PosInf /\
            expectation p (Normal o f o real o N) <> NegInf
Proof
    NTAC 4 STRIP_TAC
 >> MATCH_MP_TAC expectation_finite >> simp []
 >> irule integrable_bounded_continuous
...
Theorem lp_space_imp_L1_space :
    !m p f. measure_space m /\ measure m (m_space m) < PosInf ==>
            (1 <= p /\ p <> PosInf /\ f IN lp_space p m ==> f IN L1_space m)
Proof
    RW_TAC std_ss []
 >> Cases_on ‘p = 1’
 >- (rw [lp_space_def])
 ...
 >> Know ‘measure m (m_space m) powr (1 - p⁻¹) <> PosInf /\ measure m (m_space m) powr (1 - p⁻¹) <> NegInf’
 >- (Know ‘0 <= measure m (m_space m)’
     >- (‘m_space m IN measurable_sets m’ by METIS_TAC [MEASURE_SPACE_SPACE] \\
     ...
     ‘Normal a powr (1 - p⁻¹) <> NegInf’ by METIS_TAC [extreal_0_simps, lt_trans] \\
     art [] \\
...
val clt_tactic1 =
    ...
    >- (simp [Abbr ‘s’] \\
        ...
        irule sqrt_infty \\
        CONJ_TAC
        >- (Suff ‘second_moments p X n < PosInf’
            >- (METIS_TAC [lt_imp_ne]) \\
...
Theorem real_random_variable_prod_measure_fst[local] :
    !p q X (N :num).
      ...
      (!i. i < N ==> real_random_variable (X i o FST) (p CROSS q))
Proof
  rpt STRIP_TAC
  >> Know ‘(p_space (p × q),events (p × q)) =
           (p_space p,events p) × (p_space q,events q)’
...
Theorem real_random_variable_prod_measure_snd[local] :
  !p q X (N :num).
    ...
    (!i. i < N ==> real_random_variable (X i o SND) (p CROSS q))
Proof
  rpt STRIP_TAC
  >> Know ‘(p_space (p × q),events (p × q)) =
           (p_space p,events p) × (p_space q,events q)’
...
Theorem expectation_multidimentional_compose_fst[local] :
  !p q f.
    ...
    (!x. x IN p_space p ==> f x <> PosInf /\ f x <> NegInf) /\ integrable p f ==>
    expectation p f = expectation (p × q) (f o FST)
Proof
  rpt STRIP_TAC
...
Theorem expectation_multidimentional_compose_snd[local] :
  !p q f.
    prob_space p /\ prob_space (q :'a list m_space) /\
    (!x. x IN p_space q ==> f x <> PosInf /\ f x <> NegInf) /\ integrable q f ==>
    expectation q f = expectation (p × q) (f o SND)
Proof
  rpt STRIP_TAC
  >> (MP_TAC o (Q.SPECL [‘p’, ‘q’, ‘f’]) o
...
Theorem integrable_bounded_continuous_rv[local] :
    !p X f. prob_space p /\
            random_variable X p borel /\
            f IN C_b ==>
            integrable p (Normal o f o X)
Proof
  rw [C_b_def, bounded_def]
  >> MP_TAC (Q.SPECL [‘p’, ‘λx. Normal a’, ‘Normal o (f: real -> real) o X’]
                     integrable_bounded)
 >> fs [prob_space_def]
 >> impl_tac
 >- (CONJ_TAC
  ...
  !p X Y f s n.
    ...
    (!j. j < n ==>
         (!Z. Z = (λj x. if x IN p_space p then
              ...
              expectation p (Normal o f o real o (λx. ∑ (λi. X i x) (count n) / s n)) -
                          expectation p (Normal o f o real o (λx. ∑ (λi. Y i x) (count n) / s n)) =
              ∑ (λj. expectation p (Normal o f o real o (λx. (X j x + Z j x) / s n)) -
                                 expectation p (Normal o f o real o (λx. (Y j x + Z j x) / s n))) (count n)))
Proof
  rw [] >> rename1 ‘k < n’
  >> Q.ABBREV_TAC ‘Z = (λj x. if x IN p_space p then
                                (∑ (λi. Y i x) (count j) + ∑ (λi. X i x) (count n DIFF count1 j))
  ...
  >> DISCH_TAC
  >> Know ‘integrable p
           (Normal o f o real o (λx. ∑ (λi. Y i x) (count n) / s n))’
  ...
  >> Q.ABBREV_TAC ‘f_X = Normal o f o real o S_X’
  >> Q.ABBREV_TAC ‘f_Y = Normal o f o real o S_Y’
  >> Q.ABBREV_TAC ‘G = (λj x.
                          if x IN p_space p then
                            ∑ (λi. Y i x) (count j) +
                            ∑ (λi. X i x) (count n DIFF count j)
                          else 0)’
  >> MP_TAC (Q.SPECL [‘p’, ‘X’, ‘Y’, ‘G’, ‘f’, ‘n’] clt_partial_sum_lemma)
  >> rw []
  >> Q.ABBREV_TAC ‘g = λj. Normal o f o real o (λx. G j x / s n)’
  ...
  >> fs [Abbr ‘g’]
  >> Know ‘expectation p (Normal o f o real o (λx. G j x / s n)) =
           expectation p (Normal o f o real o (λx. (X j x + Z j x) / s n))’
  >- (irule expectation_cong >> simp [] \\
      Know ‘!x. x IN p_space p ==> G j x = (X j x + Z j x)’
      >- (simp [Abbr ‘G’, Abbr ‘Z’] \\
          ...
          POP_ASSUM (fs o wrap) \\
          ‘!x. x IN p_space p ==> X j x + ∑ (λi. Y i x) (count j) + ∑ (λi. X i x) (count n DIFF count1 j) =
                               ∑ (λi. Y i x) (count j) + X j x + ∑ (λi. X i x) (count n DIFF count1 j)’
            by METIS_TAC [add_comm] \\
          POP_ASSUM (fs o wrap) \\
          ‘!x. x IN p_space p ==> ∑ (λi. Y i x) (count j) + X j x + ∑ (λi. X i x) (count n DIFF count1 j) =
                               ∑ (λi. Y i x) (count j) + (X j x + ∑ (λi. X i x) (count n DIFF count1 j))’
            by METIS_TAC [GSYM add_assoc] \\
          POP_ASSUM (fs o wrap) \\
          Know ‘!x. x IN p_space p ==> ∑ (λi. X i x) (count n DIFF count j) =
                                    (X j x + ∑ (λi. X i x) (count n DIFF count1 j))’
          >- (GEN_TAC >> STRIP_TAC \\
              Q.PAT_X_ASSUM ‘!x. x IN p_space p ==>
                                 ∑ (λi. X i x) (count n DIFF count1 j) <> PosInf /\ _’
              ...
              Know ‘!x'. x' IN {j} UNION (count n DIFF count1 j) ==> (λi. X i x) x' <> PosInf’
              >- (‘count n DIFF count j = {j} UNION (count n DIFF count1 j)’ by rw [Once EXTENSION] \\
                  GEN_TAC >> STRIP_TAC \\
                  BETA_TAC >> fs [] \\
                  Q.PAT_X_ASSUM ‘!i. i < n ==> real_random_variable (X i) p /\ _’
      ...
      DISCH_THEN (fs o wrap))
  >> Rewr
  >> Know ‘expectation p (Normal o f o real o (λx. G (j + 1) x / s n)) =
           expectation p (Normal o f o real o (λx. (Y j x + Z j x) / s n))’
  >- (irule expectation_cong >> simp [] \\
      Know ‘!x. x IN p_space p ==> G (j + 1) x = Y j x + Z j x’
      >- (simp [Abbr ‘G’, Abbr ‘Z’] \\
          ...
          >- (GEN_TAC >> STRIP_TAC \\
              ‘count (j + 1) = count j UNION {j}’ by rw [Once EXTENSION] \\
              POP_ORW \\
              Know ‘∑ (λi. Y i x) (count j UNION {j}) = ∑ (λi. Y i x) (count j) + ∑ (λi. Y i x) {j}’
              >- (irule EXTREAL_SUM_IMAGE_DISJOINT_UNION \\
                  simp [] >> DISJ2_TAC >> rw []
                  >- (Q.PAT_X_ASSUM ‘!i. i < n ==> real_random_variable (X i) p /\ _’
                  ...
                  Q.PAT_X_ASSUM ‘!x. x IN p_space p ==> Y j x <> PosInf /\ Y j x <> NegInf’
                   (STRIP_ASSUME_TAC o Q.SPEC ‘x’) >> fs []) \\
              Rewr \\
              NTAC 2 (Q.PAT_X_ASSUM ‘!x. x IN p_space p ==> _’ (STRIP_ASSUME_TAC o Q.SPEC ‘x’)) \\
              gs [add_not_infty]) \\
          DISCH_TAC \\
          Know ‘!x. x IN p_space p ==>
                    ∑ (λi. X i x) (count n DIFF count1 j) <> PosInf /\
                    ∑ (λi. X i x) (count n DIFF count1 j) <> NegInf’
          >- (GEN_TAC >> STRIP_TAC >> CONJ_TAC
              >- (irule EXTREAL_SUM_IMAGE_NOT_POSINF >> simp [] \\
                  Q.X_GEN_TAC ‘i’ >> STRIP_TAC \\
          ...
          ‘!x. x IN p_space p ==>
            ...
            by  METIS_TAC [add_assoc] \\
          POP_ASSUM (fs o wrap) \\
          Know ‘!x. x IN p_space p ==> ∑ (λi. Y i x) (count (j + 1)) = Y j x + ∑ (λi. Y i x) (count j)’
          >- (GEN_TAC >> STRIP_TAC \\
              Q.PAT_X_ASSUM ‘!x. x IN p_space p ==>
                                 ∑ (λi. Y i x) (count (j + 1)) <> PosInf /\ _’
               (STRIP_ASSUME_TAC o Q.SPEC ‘x’) >> gs [] \\
...
Theorem IN_MEASURABLE_BOREL_CDIV :
    !a f g z.
      sigma_algebra a /\ f IN Borel_measurable a /\ z <> 0 /\
...
val taylor_tactic3 =
    Know ‘expectation p (λx. abs (X j x)³) <> NegInf /\
          expectation p (λx. abs (X j x)³) <> PosInf’
    >- (MP_TAC (Q.SPECL [‘p’, ‘λx. abs (X (j :num) x)³’] expectation_finite) \\
        simp [] \\
        Q.PAT_X_ASSUM ‘!j. j < n ==>
                           real_random_variable (X j) p /\ _’ (STRIP_ASSUME_TAC o Q.SPEC ‘j’) >> gs []) \\
    DISCH_TAC \\
    Know ‘expectation p (λx. abs (Y j x)³) <> NegInf /\
          expectation p (λx. abs (Y j x)³) <> PosInf’
...
Theorem integral_eq_third_moment[local] :
    !p f X Y Z r.
      ...
      ∫ p (λx. abs (X x)³) = 0 /\
      ∫ p (λx. abs (Y x)³) = 0 ==>
      ∫ p (Normal o f o real o (λx. (X x + Z x) / Normal r)) =
      ∫ p (Normal o f o real o (λx. (Y x + Z x) / Normal r))
Proof
    rw [real_random_variable_def, random_variable_def, p_space_def, events_def]
 >> MP_TAC (Q.SPECL [‘p’, ‘λx. (X x)³’] integral_abs_eq_0)
 >> fs [prob_space_def]
 >> Know ‘(λx. (X x)³) IN Borel_measurable (measurable_space p)’
 >- (irule IN_BOREL_MEASURABLE_POW \\
 ...
 >> simp []
 >> Know ‘(λx. (Y x)³) IN Borel_measurable (measurable_space p)’
 >- (irule IN_BOREL_MEASURABLE_POW \\
     simp [MEASURE_SPACE_SIGMA_ALGEBRA] \\
 ...
 >> STRIP_TAC
 >> MP_TAC (Q.SPECL [‘p’, ‘λx. (((X :'a -> extreal) x + Z x) / Normal r = (Z :'a -> extreal) x / Normal r)’,
                     ‘λx. ((X :'a -> extreal) x + Z x = (Z :'a -> extreal) x)’] AE_cong)
 >> simp [] >> STRIP_TAC
 ...
 >- (rw [] >> ‘?a. Y x = Normal a’ by METIS_TAC [extreal_cases] \\
     ...
     METIS_TAC [REAL_DIV_RMUL, REAL_LT_IMP_NE])
 >> STRIP_TAC
 >> MP_TAC (Q.SPECL [‘p’, ‘λx. (((Y :'a -> extreal) x + Z x) / Normal r = (Z :'a -> extreal) x / Normal r)’,
                     ‘λx. (Y x + Z x = (Z :'a -> extreal) x)’] AE_cong)
 >> simp [] >> STRIP_TAC
...
Theorem sup_abs_diff3_nonneg[local]:
    !f. f IN CnR 3 ==>
        0 <= sup (IMAGE (λt. abs (Normal (diffn 3 f t))) univ(:real))
Proof
  rw [CnR_def, le_sup]
...
Theorem taylor_remainder_bound_lemma[local] :
  !f. f IN CnR 3 ==>
      (!t. abs (Normal (diffn 3 f t)) <= sup (IMAGE (λt. abs (Normal (diffn 3 f t))) univ(:real)))
Proof
  rw [le_sup]
  >> POP_ASSUM (STRIP_ASSUME_TAC o Q.SPEC ‘abs (Normal (diffn 3 f t))’)
  >> Know ‘?t'. abs (Normal (diffn 3 f t)) = abs (Normal (diffn 3 f t'))’
...
Theorem clt_sup_finite[local] :
    !f. f IN CnR 3 ==> sup (IMAGE (λx. abs (Normal (diffn 3 f x))) UNIV) <> PosInf
Proof
  rw []
  >> Know ‘abs (sup (IMAGE (λx. abs (Normal (diffn 3 f x))) univ(:real))) =
                sup (IMAGE (λx. abs (Normal (diffn 3 f x))) univ(:real))’
...
Theorem TAYLOR_REMAINDER_THIRD_ORDER_BOUND[local] :
    !f a h M t.
      f IN CnR 3 /\
      M = sup (IMAGE (λt. abs (Normal (diffn 3 f t))) univ(:real)) ==>
      abs (Normal (1 / 6 * (h³ * diffn 3 f t))) <= M / 6 * abs (Normal h)³
Proof
    rw []
 >> Q.ABBREV_TAC ‘M = sup (IMAGE (λt. abs (Normal (diffn 3 f t))) univ(:real))’
...
Theorem clt_real_random_variable_compose[local] :
    !p X f.
      prob_space p /\ real_random_variable X p /\ f IN CnR 3 ==>
      real_random_variable (Normal o f o real o X) p
Proof
  rw [real_random_variable, prob_space_def, p_space_def, events_def]
  >> irule IN_MEASURABLE_BOREL_IMP_BOREL
  >> irule in_measurable_borel_comp_borel
  >> qexistsl [‘f’, ‘real o X’] >> simp []
  >> ‘f IN borel_measurable borel’ by METIS_TAC [in_borel_measurable_CnR]
  >> simp []
...
Theorem CnR_subset_class_lipschitz :
    !n. 1 <= n ==> CnR n SUBSET C_bounded_lipschitz
Proof
...
Theorem diff_Lipschitz_from_CnR :
    !n f. 2 <= n /\ f IN CnR n ==> Lipschitz_fun (diffn (n - 1) f)
Proof
    rw [CnR_def]
 >> MATCH_MP_TAC higher_differentiable_bounded_imp_lipschitz
 >> CONJ_TAC  >- (rw [] \\
...
Theorem CONTINUOUS_ON_COMPOSE_UNIV :
    !f g. f continuous_on univ(:real) /\ g continuous_on univ(:real) ==>
          (f o g) continuous_on univ(:real)
Proof
    rpt STRIP_TAC
...
Theorem bounded_o :
    !f g. bounded (IMAGE f univ(:real)) /\ bounded (IMAGE g univ(:real)) ==>
          bounded (IMAGE (f o g) univ(:real))
Proof
    rw [bounded_def]
 >> qexists ‘max a a'’
 >> rename1 ‘max a b’  >> rw []
...
Theorem C_b_o :
    !f g. f IN C_b /\ g IN C_b ==> f o g IN C_b
Proof
...
Theorem sup_abs_diffn_nonneg :
   !f n. f IN CnR n ==>
         0 <= sup (IMAGE (λt. abs (Normal (diffn n f t))) univ(:real))
Proof
  rw [CnR_def, le_sup]
...
Theorem sup_abs_finite :
    !f (n :num). f IN CnR n ==> sup (IMAGE (λx. abs (Normal (diffn n f x))) UNIV) <> PosInf
Proof
  rw []
  >> Know ‘abs (sup (IMAGE (λx. abs (Normal (diffn n f x))) univ(:real))) =
           sup (IMAGE (λx. abs (Normal (diffn n f x))) univ(:real))’
...
Proof
 ...
 >- (simp [prob_space_def] \\
     ...
     Q.ABBREV_TAC ‘M = sup (IMAGE (λt. abs (Normal (diffn 1 f t))) univ(:real))’ \\
     ...
     (* (λx. Normal (diffn 1 f (real (Z x)))²) IN Borel_measurable (measurable_space p) *)
...
Proof
 ...
 >> rw [GSYM extreal_mul_eq]
 >> MATCH_MP_TAC integrable_bounded
 >> Q.ABBREV_TAC ‘M = sup (IMAGE (λt. abs (Normal (diffn 2 f t))) univ(:real))’
 ...
 >- (Know ‘0 <= M’
    ...
    >- (irule IN_MEASURABLE_BOREL_MUL' \\
        ...
        fs [real_random_variable, p_space_def, events_def] \\
        (* (λx. Normal (diffn 2 f (real (Z x)))) IN  Borel_measurable (measurable_space p) *)
        CONJ_TAC
        >- (rw [GSYM o_DEF] \\
            MATCH_MP_TAC IN_MEASURABLE_BOREL_IMP_BOREL' \\
            simp [MEASURE_SPACE_SIGMA_ALGEBRA] \\
            ...
            MATCH_MP_TAC in_borel_measurable_diffn \\
            qexists ‘3’ >> gs []) \\
        (* (λx. (X x)²) IN Borel_measurable (measurable_space p) *)
        MATCH_MP_TAC IN_BOREL_MEASURABLE_POW \\
        qexistsl [‘2’, ‘X’] \\
        ASM_SIMP_TAC std_ss [MEASURE_SPACE_SIGMA_ALGEBRA] \\
        ‘space (measurable_space p) = m_space p’ by fs [] >> gs [])
...
Proof
         ...
         >- (MP_TAC (Q.SPECL [‘p’, ‘X’, ‘Z’, ‘Borel’, ‘Borel’,
             ...
             >- (fs [real_random_variable_def] \\
                 ...
                 >- (METIS_TAC [IN_MEASURABLE_BOREL_IMP_BOREL', real_in_borel_measurable,
                                SIGMA_ALGEBRA_BOREL])\\
                 (* Normal o diffn 1 f o real IN Borel_measurable Borel *)
                      MATCH_MP_TAC IN_MEASURABLE_BOREL_IMP_BOREL' \\
                 simp [SIGMA_ALGEBRA_BOREL] \\
                 MATCH_MP_TAC MEASURABLE_COMP \\
                 qexists ‘borel’ \\
 ...
 >> qexists ‘expectation p (abs o (λx. H x - B x))’ >> gs []
...
Theorem taylor_diffn_expectation_bound :
    !p X Y Z f M.
      ...
      abs (expectation p (Normal o f o real o (λx. X x + Z x)) -
           expectation p (Normal o f o real o (λx. Y x + Z x)))
...
Theorem taylor_diffn_expectation_bound_scaled[local] :
    !p X Y Z f M s n.
      ...
      M = sup (IMAGE (λt. abs (Normal (diffn 3 f t))) UNIV) ==>
      (!(j :num). j < n ==>
           abs (expectation p (Normal o f o real o (λx. (X j x + Z j x) / s n)) -
                   expectation p (Normal o f o real o (λx. (Y j x + Z j x) / s n)))
           <= M / (6 * (s n pow 3)) *
...
Proof
 ...
 >> STRIP_TAC
 >> Know ‘expectation p
          (Normal o f o real o (λx. (X j x + Z j x) / Normal r)) =
          expectation p (Normal o f o real o (λx. X' j x + Z' j x))’
 ...
 >> STRIP_TAC
 >> Know ‘expectation p
          (Normal o f o real o (λx. (Y j x + Z j x) / Normal r)) =
          expectation p (Normal o f o real o (λx. Y' j x + Z' j x))’
 ...
 >- (simp [] >> rename1 ‘i < n’ \\
     ...
     DISCH_TAC \\
     (* expectation p (λx. (abs (X' i x))³) < PosInf *)
     STRONG_CONJ_TAC
     >- (rw [Abbr ‘X'’, GSYM pow_abs] \\
         ...
         impl_tac >- (fs [prob_space_def, p_space_def, real_random_variable] \\
                      rw [] \\
                      Q.PAT_X_ASSUM ‘!x. x IN m_space p ==> X i x <> NegInf /\ X i x <> PosInf’
                       (STRIP_ASSUME_TAC o Q.SPEC ‘x’) \\
     ...
     DISCH_TAC \\
     (* expectation p (λx. (abs (Y' i x))³) < PosInf *)
     STRONG_CONJ_TAC
     >- (rw [Abbr ‘Y'’, GSYM pow_abs] \\
         ...
         impl_tac >- (fs [prob_space_def, p_space_def, real_random_variable] \\
                      rw [] \\
                      Q.PAT_X_ASSUM ‘!x. x IN m_space p ==> Y i x <> NegInf /\ Y i x <> PosInf’
                       (STRIP_ASSUME_TAC o Q.SPEC ‘x’) \\
                      rw [extreal_pow_def, pow_div] \\
                      MATCH_MP_TAC abs_div \\
                      ‘Normal r³ <> 0’ by rw [GSYM extreal_pow_def] >> simp [] \\
     ...
     >- (rw [Abbr ‘Y'’, Abbr ‘X'’, pow_div] \\
         ...
         >- (MP_TAC (Q.SPECL [‘p’, ‘λx. (X (i :num) x) pow 2’, ‘r pow 2’] expectation_cdiv) \\
             ...
             fs [real_random_variable] \\
             Q.PAT_X_ASSUM ‘!x. x IN p_space p ==> X i x <> NegInf /\ X i x <> PosInf’
              (STRIP_ASSUME_TAC o Q.SPEC ‘x’) \\
         ...
         >- (MP_TAC (Q.SPECL [‘p’, ‘λx. (Y (i :num) x) pow 2’, ‘r pow 2’] expectation_cdiv) \\
             ...
             MATCH_MP_TAC expectation_cong >> rw [] \\
             fs [real_random_variable] \\
             Q.PAT_X_ASSUM ‘!x. x IN p_space p ==> Y i x <> NegInf /\ Y i x <> PosInf’
              (STRIP_ASSUME_TAC o Q.SPEC ‘x’) \\
             rw [extreal_pow_def, pow_div]) >> Rewr \\
         fs []) \\
     DISCH_TAC \\
     ...
     >- (rw [Abbr ‘X'’, Abbr ‘Z'’] \\
         (MP_TAC o (Q.SPECL [‘p’, ‘X (i :num)’, ‘Z (i :num)’, ‘Borel’, ‘Borel’,
                 ...
                 (INST_TYPE [beta |-> “:extreal”])) indep_rv_cong \\
         Suff ‘(λx. x / Normal r) IN Borel_measurable Borel’
 ...
 >- (rw [Abbr ‘X'’, Abbr ‘Y'’] \\
     ...
     >- (‘(Normal r) pow 3 = Normal (r pow 3)’ by rw [extreal_pow_def] \\
         ...
         >- (rw [] \\
             Q.PAT_X_ASSUM ‘!j. j < n ==>
                                real_random_variable (X j) p /\ _’ (STRIP_ASSUME_TAC o Q.SPEC ‘j’) >> gs [] \\
             fs [real_random_variable_def] \\
             Q.PAT_X_ASSUM ‘!x. x IN p_space p ==> X j x <> NegInf /\ _’ (STRIP_ASSUME_TAC o Q.SPEC ‘x’) \\
     ...
     >- (‘(Normal r) pow 3 = Normal (r pow 3)’ by rw [extreal_pow_def] \\
         ...
         >- (rw [] \\
             Q.PAT_X_ASSUM ‘!j. j < n ==>
                                real_random_variable (X j) p /\ _’ (STRIP_ASSUME_TAC o Q.SPEC ‘j’) >> gs [] \\
             fs [real_random_variable_def] \\
             Q.PAT_X_ASSUM ‘!x. x IN p_space p ==> Y j x <> NegInf /\ _’ (STRIP_ASSUME_TAC o Q.SPEC ‘x’) \\
     ...
     >- (irule div_eq_mul_rinv \\
         ...
         ‘h <> NegInf /\ h <> PosInf’ by METIS_TAC [Abbr ‘h’, add_not_infty] >> simp [] \\
...
Theorem clt_lindeberg_taylor_error_bound[local] :
  !r X Y Z f M s n.
    ...
    M = sup (IMAGE (λt. abs (Normal (diffn 3 f t))) UNIV) /\
    0 < s n /\ s n <> PosInf /\ s n <> NegInf ==>
    abs (∑ (λj. expectation r (Normal o f o real o (λx. (X j x + Z j x) / s n)) -
                            expectation r (Normal o f o real o (λx. (Y j x + Z j x) / s n))) (count n)) <=
...
Proof
 ...
 >> Q.ABBREV_TAC ‘h = λj. expectation r (Normal o f o real o (λx. (X j x + Z j x) / s n)) -
                          expectation r (Normal o f o real o (λx. (Y j x + Z j x) / s n))’
 >> gs []
...
Theorem measure_density :
    !m f s. measure (density m f) s = pos_fn_integral m (λx. f x * 𝟙 s x)
Proof
...
Theorem measurable_preimage_borel :
    !f m.
      f IN borel_measurable (measurable_space m) ==>
      !s. s IN subsets borel ==> PREIMAGE f s INTER m_space m IN measurable_sets m
Proof
    rpt STRIP_TAC
 >> (MP_TAC o (Q.SPECL [‘measurable_space m’, ‘borel’, ‘f’]) o
...
Theorem distribution_eq' :
    !p X Y mu sig s.
      ...
      (s IN subsets borel ==> distribution p X s = distribution p Y s)
Proof
    rw [distribution_def]
 >> AP_TERM_TAC
...
Proof
 ...
 >> rw [normal_rv_def]
 >> EQ_TAC >> rw []
 >> Q.PAT_X_ASSUM ‘!s. s IN subsets borel ==> _’ (STRIP_ASSUME_TAC o Q.SPEC ‘s’) >> gs []
...
Theorem normal_rv_eq[local] :
    !p X Y mu sig.
      ...
      (!x. x IN p_space p ==> Y x = X x) ==> normal_rv Y p mu sig
Proof
 ...
 >- (irule in_borel_measurable_add \\
     ...
     qexists ‘0’ >> simp [])
 >> Q.PAT_X_ASSUM ‘!s. s IN subsets borel ==> _’ (STRIP_ASSUME_TAC o Q.SPEC ‘s’)
...
Proof
 ...
 >> STRONG_CONJ_TAC
 >- (GEN_TAC >> STRIP_TAC \\
     Q.PAT_X_ASSUM ‘!x. x IN p_space p ==> _’ (STRIP_ASSUME_TAC o Q.SPEC ‘x’) \\
...
Theorem MEASURABLE_EL'[local] :
    !p p' n i.
      ...
      EL i IN measurable (measurable_space p') (measurable_space p)
Proof
    rw []
...
Theorem in_measurable_borel_comp :
    !a b f g h.
      f IN borel_measurable b /\ g IN measurable a b /\
...
val indep_vars_tactic1 =
    Know ‘!i x. i < n /\ x IN p_space p' ==> EL i x IN p_space p’
    >- (rw [p_space_def, m_space_def, Once EXTENSION] \\
        ...
        ‘x IN rectangle (λi. p_space p) n’ by fs [p_space_def] \\
        fs [IN_list_rectangle]) >> DISCH_TAC ;

Theorem existence_of_indep_vars :
    !(p :'a m_space) N (D :num -> real) n.
...
Proof
 ...
 >- (indep_vars_tactic1 \\
     ...
     >- (fs [random_variable_def, p_space_def, events_def] \\
         ...
         POP_ORW \\
         Q.PAT_X_ASSUM ‘ !i'. i' < n ==> (!x. x IN m_space p ==> Y i' x <> PosInf /\ Y i' x <> NegInf) /\ _’
     ...
     POP_ORW \\
     Q.ABBREV_TAC ‘h = λj. if j = i then PREIMAGE (f j) s INTER p_space p
                              else p_space p’ \\
     Know ‘PREIMAGE (f i o EL i) s INTER p_space p' = rectangle h n’
     >- (‘space (measurable_space p') = rectangle (λi. space (measurable_space p)) n’
     ...
     Q.PAT_X_ASSUM ‘!i'. i' < n ==> (!x. x IN p_space p ==> Y i' x <> PosInf /\ Y i' x <> NegInf) /\ _’
      (STRIP_ASSUME_TAC o Q.SPEC ‘i’) >> gs [] \\
     Q.PAT_X_ASSUM ‘!s'. s' IN subsets borel ==> _’ (STRIP_ASSUME_TAC o Q.SPEC ‘s’) >> gs [] \\
     POP_ASSUM (rw  o wrap o SYM) \\
     Q.PAT_X_ASSUM ‘!h. (!i. i < n ==> h i IN events p) ==> _’ (STRIP_ASSUME_TAC o Q.SPEC ‘h’) \\
     Know ‘!i. i < n ==> h i IN events p’
 ...
 >> Q.ABBREV_TAC ‘h = λj. if j IN N' then PREIMAGE (Y j) (E j) INTER p_space p else p_space p’
 >> Know ‘!j. j IN N' ==> PREIMAGE (Y' j) (E j) INTER p_space p' =
                        PREIMAGE (EL j) (PREIMAGE (Y j) (E j)) INTER p_space p'’
 >- (rw [Abbr ‘Y'’, PREIMAGE_def, Once EXTENSION]) >> DISCH_TAC
 >> Know ‘!j. j IN N' ==> PREIMAGE (EL j) (PREIMAGE (Y j) (E j)) INTER p_space p' =
                        PREIMAGE (EL j) (h j) INTER p_space p' ’
 ...
 >> DISCH_TAC
 >> fs [PREIMAGE_o, o_DEF]
 >> Know ‘BIGINTER (IMAGE (λj. PREIMAGE (Y' j) (E j) INTER p_space p') N') =
          BIGINTER (IMAGE (λj. PREIMAGE (EL j) (h j) INTER p_space p') N')’
 >-(AP_TERM_TAC >> rw [IMAGE_DEF, Once EXTENSION] \\
    EQ_TAC >> rw [] >> qexists ‘j’ >> rw [])
 >> Rewr
 >> Know ‘∏ (λj. prob p' (PREIMAGE (Y' j) (E j) INTER p_space p')) N' =
          ∏ (λj. prob p' (PREIMAGE (EL j) (h j) INTER p_space p')) N'’
 >- (irule EXTREAL_PROD_IMAGE_EQ >> METIS_TAC []) >> Rewr
 >> NTAC 2 POP_ORW
 >> Know ‘!x. x IN p_space p' ==> LENGTH x = n’
 >- (‘space (measurable_space p') = rectangle (λi. space (measurable_space p)) n’
       by METIS_TAC [space_sigma_lists, p_space_def] \\
     ‘p_space p' = rectangle (λi. p_space p) n’ by fs [p_space_def] \\
     fs [list_rectangle_def]) >> DISCH_TAC
 >> Q.ABBREV_TAC ‘f = λj. PREIMAGE (EL j) (h j) INTER p_space p'’ >> gs []
 >> Q.ABBREV_TAC ‘h' = (λj. if j IN N' then h j else p_space p)’
 >> rw [GSYM IMAGE_o]
 >> Know ‘BIGINTER (IMAGE f N') = rectangle h' n INTER p_space p'’
 >- (rw [BIGINTER_IMAGE, Abbr ‘f’, Abbr ‘h'’, list_rectangle_def] \\
     ...
     >- (fs [GSYM MEMBER_NOT_EMPTY] \\
        ...
        >- (Cases_on ‘j NOTIN N'’ >> gs [] \\
            ...
            fs [GSYM MEMBER_NOT_EMPTY] \\
            Q.PAT_X_ASSUM ‘!j. j IN N' ==> EL j x IN h j /\ x IN p_space p'’ (STRIP_ASSUME_TAC o Q.SPEC ‘x'’) >> gs [])
     >- (fs [GSYM MEMBER_NOT_EMPTY] \\
         Q.PAT_X_ASSUM ‘!j. j IN N' ==> EL j x IN h j /\ x IN p_space p'’ (STRIP_ASSUME_TAC o Q.SPEC ‘x'’) >> gs [])
     >- (‘j < LENGTH x’ by fs [SUBSET_DEF, count_def] \\
         Q.PAT_X_ASSUM ‘!j. j < LENGTH x ==> EL j x IN if j IN N' then h j else p_space p’
          (STRIP_ASSUME_TAC o Q.SPEC ‘j’) >> gs [])) >> Rewr
 >> Know ‘prob p' (rectangle h' n INTER p_space p') = prob p' (rectangle h' n)’
 >- (Know ‘rectangle h' n SUBSET p_space p'’
     >- (‘space (measurable_space p') = rectangle (λi. space (measurable_space p)) n’
           by METIS_TAC [space_sigma_lists, p_space_def] \\
         ‘p_space p' = rectangle (λi. p_space p) n’ by fs [p_space_def] \\
     ...
     ‘rectangle h' n INTER p_space p' = rectangle h' n’ by METIS_TAC [SUBSET_INTER1] \\
     POP_ORW >> rw []) >> Rewr
 ...
 >- (Q.PAT_X_ASSUM ‘!h'. (!i. i < n ==> h' i IN events p) ==> _’ (STRIP_ASSUME_TAC o Q.SPEC ‘h'’) \\
     ...
     >- (POP_ORW \\
         ...
         >- (fs [p_space_def, events_def, random_variable_def] \\
             ...
             Know ‘Y i IN Borel_measurable (measurable_space p)’
 ...
 >- (rw [Abbr ‘f’, Abbr ‘h’] \\
     Q.PAT_X_ASSUM ‘!x. x IN N' ==> E x IN subsets Borel’ (STRIP_ASSUME_TAC o Q.SPEC ‘j’) >> gs [] \\
     Q.ABBREV_TAC ‘A = PREIMAGE (Y j) (E j) INTER p_space p’ \\
     Q.ABBREV_TAC ‘f = λj. EL j’ >> gs [] \\
     Q.ABBREV_TAC ‘g = λi. if i = j then A else p_space p’ \\
     Know ‘PREIMAGE (EL j) A INTER p_space p' = rectangle g n’
     >- (‘space (measurable_space p') = rectangle (λi. space (measurable_space p)) n’
           by METIS_TAC [space_sigma_lists, p_space_def] \\
         ‘p_space p' = rectangle (λi. p_space p) n’ by fs [p_space_def] \\
         ...
         Q.PAT_X_ASSUM ‘!i. i < LENGTH x' ==> f i x' IN if i = j then A else m_space p’
          (STRIP_ASSUME_TAC o Q.SPEC ‘i’) >> gs [] \\
         Cases_on ‘i = j’ >> rw [Abbr ‘A’] >> METIS_TAC []) >> rw [Abbr ‘f’] \\
     Q.PAT_X_ASSUM ‘!h'. (!i. i < n ==> h' i IN events p) ==> _’
      (STRIP_ASSUME_TAC o Q.SPEC ‘g’) \\
     Know ‘(!i. i < n ==> g i IN events p)’
     ...
     ‘!x. prob p (if x = j then A else p_space p) =
          if x IN {j} then prob p A else 1’ by rw [PROB_UNIV] >> POP_ORW \\
 ...
 >- (irule EXTREAL_PROD_IMAGE_EQ >> METIS_TAC []) >> Rewr
 >> rw [Abbr ‘h'’]
 >> ‘!j. prob p (if j IN N' then h j else p_space p) =
         if j IN N' then prob p (h j) else 1’ by rw [PROB_UNIV]
 >> POP_ORW
...
Theorem PROB_BIGINTER_INDEP :
    !p X N E. indep_vars p X (λi. Borel) N /\ FINITE N /\ N <> {} /\
              (!i. i IN N ==> E i IN subsets Borel) ==>
              prob p (BIGINTER (IMAGE (λi. PREIMAGE (X i) (E i) INTER p_space p) N)) =
              ∏ (λi. prob p (PREIMAGE (X i) (E i) INTER p_space p)) N
Proof
    rw [indep_vars_def, IN_DFUNSET]
...
Theorem MEASURABLE_BIGINTER :
    !m s.
      measure_space m /\ FINITE s /\ s <> {} /\ (!x. x IN s ==> x IN measurable_sets m) ==>
      BIGINTER s IN measurable_sets m
Proof
    rpt STRIP_TAC
 >> MP_TAC (Q.SPECL [‘measurable_space m’, ‘s’] SIGMA_ALGEBRA_FINITE_INTER')
 >> simp [MEASURE_SPACE_SIGMA_ALGEBRA]
 >> Suff ‘s SUBSET measurable_sets m’ >- (rw [])
...
Theorem MEASURABLE_BIGINTER_IMAGE_PREIMAGE_BOREL :
    !f m J.
      measure_space m /\
      FINITE J /\ J <> {} /\
      (!i. i IN J ==> f i IN Borel_measurable (measurable_space m)) ==>
      (!s. s IN subsets Borel ==>
      BIGINTER (IMAGE (λi. PREIMAGE (f i) s INTER m_space m) J) IN measurable_sets m)
Proof
  rpt STRIP_TAC
  >> (MP_TAC o (Q.SPECL [‘measurable_space m’, ‘IMAGE (λi. PREIMAGE ((f :'a -> 'b -> extreal) i) s INTER m_space m) J’]) o
               (INST_TYPE [alpha |-> “:'b”])) SIGMA_ALGEBRA_FINITE_INTER'
  >> simp [MEASURE_SPACE_SIGMA_ALGEBRA]
  >> Suff ‘IMAGE (λi. PREIMAGE (f i) s INTER m_space m) J SUBSET measurable_sets m’ >- (rw [])
...
Theorem PROB_FST :
    !p q A.
      prob_space p /\ prob_space q /\
      A SUBSET p_space p /\ A IN events p ==>
      prob (p × q) (PREIMAGE FST A INTER p_space (p × q)) = prob p A
Proof
    rpt STRIP_TAC
 >> Know ‘PREIMAGE FST A INTER p_space (p × q) = A CROSS p_space q’
...
Theorem PROB_SND :
    !p q A.
      prob_space p /\ prob_space q /\
      A SUBSET p_space q /\ A IN events q ==>
      prob (p × q) (PREIMAGE SND A INTER p_space (p × q)) = prob q A
Proof
    rpt STRIP_TAC
 >> Know ‘PREIMAGE SND A INTER p_space (p × q) = p_space p CROSS A’
...
Theorem BIGINTER_IMAGE_PREIMAGE_FST_LEMMA :
    !A X (N :'index set). FINITE N /\ N <> {} ==>
            BIGINTER (IMAGE (λn. PREIMAGE FST (A n) INTER X) N) =
            PREIMAGE FST (BIGINTER (IMAGE A N)) INTER X
Proof
    rpt STRIP_TAC
 ...
 >> rw [Abbr ‘B’, Abbr ‘G’]
 >- (Q.PAT_X_ASSUM ‘!P. _’ (STRIP_ASSUME_TAC o Q.SPEC ‘PREIMAGE FST ((A :'index -> 'a -> bool) x') INTER X’) \\
     fs [IN_IMAGE] \\
     Know ‘?n. PREIMAGE FST (A x') INTER X = PREIMAGE FST (A n) INTER X /\ n IN N’
     >- (qexists ‘x'’ >> fs []) >> DISCH_THEN (fs o wrap))
 >- (POP_ASSUM (STRIP_ASSUME_TAC o Q.SPEC ‘PREIMAGE FST ((A :'index -> 'a -> bool) y) INTER X’) \\
     fs [IN_IMAGE] \\
     Know ‘?n. PREIMAGE FST (A y) INTER X = PREIMAGE FST (A n) INTER X /\ n IN N’
     >- (qexists ‘y’ >> fs []) >> DISCH_THEN (fs o wrap))
 >> fs [IN_IMAGE, IN_PREIMAGE, PREIMAGE_def, INTER_DEF]
 >> Q.PAT_X_ASSUM ‘!P. _’ (STRIP_ASSUME_TAC o Q.SPEC ‘(A :'index -> 'a -> bool) n’)
 >> Know ‘?x. A n = A x /\ x IN N’
 >- (qexists ‘n’ >> fs []) >> fs []
QED

Theorem BIGINTER_IMAGE_PREIMAGE_SND_LEMMA :
    !A X (N :'index set). FINITE N /\ N <> {} ==>
                          BIGINTER (IMAGE (λn. PREIMAGE SND (A n) INTER X) N) =
                          PREIMAGE SND (BIGINTER (IMAGE A N)) INTER X
Proof
    rpt STRIP_TAC
 >> qmatch_abbrev_tac ‘BIGINTER B = G’
 >> rw [GSYM SUBSET_ANTISYM_EQ, SUBSET_DEF]
 >> fs [GSYM MEMBER_NOT_EMPTY] >> rename1 ‘y IN N’
 >> rw [Abbr ‘B’, Abbr ‘G’]
 >- (Q.PAT_X_ASSUM ‘!P. _’ (STRIP_ASSUME_TAC o Q.SPEC ‘PREIMAGE SND ((A :'index -> 'a -> bool) x') INTER X’) \\
     fs [IN_IMAGE] \\
     Know ‘?n. PREIMAGE SND (A x') INTER X = PREIMAGE SND (A n) INTER X /\ n IN N’
     >- (qexists ‘x'’ >> fs []) >> DISCH_THEN (fs o wrap))
 >- (POP_ASSUM (STRIP_ASSUME_TAC o Q.SPEC ‘PREIMAGE SND ((A :'index -> 'a -> bool) y) INTER X’) \\
     fs [IN_IMAGE] \\
     Know ‘?n. PREIMAGE SND (A y) INTER X = PREIMAGE SND (A n) INTER X /\ n IN N’
     >- (qexists ‘y’ >> fs []) >> DISCH_THEN (fs o wrap))
 >> fs [IN_IMAGE, IN_PREIMAGE, PREIMAGE_def, INTER_DEF]
 >> Q.PAT_X_ASSUM ‘!P. _’ (STRIP_ASSUME_TAC o Q.SPEC ‘(A :'index -> 'a -> bool) n’)
 >> Know ‘?x. A n = A x /\ x IN N’
...
Proof
 ...
 >> Q.PAT_X_ASSUM ‘!E N. _’ (STRIP_ASSUME_TAC o Q.SPECL [‘E’, ‘N’]) >> gs []
 >> Q.ABBREV_TAC ‘A = λn. PREIMAGE (X n) (E n) INTER p_space p’
 >> Q.ABBREV_TAC ‘B = λn. PREIMAGE (λx. X n (FST x)) (E n) INTER p_space (p × q)’
 >> Know ‘prob (p × q) (BIGINTER (IMAGE B N)) = prob p (BIGINTER (IMAGE A N))’
 >- (Suff ‘BIGINTER (IMAGE B N) = PREIMAGE FST (BIGINTER (IMAGE A N)) INTER p_space (p × q)’
     >- (rw [] \\
         MP_TAC (Q.SPECL [‘p’, ‘q’, ‘BIGINTER (IMAGE A (N :'index set))’] PROB_FST) \\
         simp [] \\
         Know ‘BIGINTER (IMAGE A N) SUBSET p_space p’
         >- (fs [GSYM MEMBER_NOT_EMPTY, Abbr ‘A’, SUBSET_DEF] >> rw [] \\
             POP_ASSUM (STRIP_ASSUME_TAC o Q.SPEC ‘PREIMAGE ((X :'index -> 'a -> extreal) x)
                                                   ((E :'index -> extreal -> bool) x) INTER p_space p’) \\
             Know ‘?n. PREIMAGE (X x) (E x) INTER p_space p =
                       PREIMAGE (X n) (E n) INTER p_space p /\ n IN N’
             >- (qexists ‘x’ >> fs []) >> DISCH_THEN (fs o wrap)) >> Rewr \\
         Know ‘BIGINTER (IMAGE A N) IN events p’
         >- (Know ‘!n. n IN N ==> A n IN events p’
             ...
             rw [IMAGE_DEF, SUBSET_DEF] \\
             Q.PAT_X_ASSUM ‘!n. n IN N ==> A n IN events p’ (STRIP_ASSUME_TAC o Q.SPEC ‘x'’) \\
             fs []) >> Rewr) \\
 ...
 >> MP_TAC (Q.SPECL [‘p’, ‘q’, ‘PREIMAGE (X x) ((E :'index -> extreal -> bool) x) INTER p_space p’] PROB_FST)
 >> simp []
 >> ‘PREIMAGE (X x) (E x) INTER p_space p IN events p’
...
Proof
    rw [indep_vars_def, IN_DFUNSET, o_DEF]
 >> Q.PAT_X_ASSUM ‘!E N. _’ (STRIP_ASSUME_TAC o Q.SPECL [‘E’, ‘N’]) >> gs []
 >> Q.ABBREV_TAC ‘A = λn. PREIMAGE (X n) (E n) INTER p_space q’
 >> Q.ABBREV_TAC ‘B = λn. PREIMAGE (λx. X n (SND x)) (E n) INTER p_space (p × q)’
 >> Know ‘prob (p × q) (BIGINTER (IMAGE B N)) = prob q (BIGINTER (IMAGE A N))’
 >- (Suff ‘BIGINTER (IMAGE B N) = PREIMAGE SND (BIGINTER (IMAGE A N)) INTER p_space (p × q)’
     >- (rw [] \\
         MP_TAC (Q.SPECL [‘p’, ‘q’, ‘BIGINTER (IMAGE A (N :'index set))’] PROB_SND) \\
         simp [] \\
         Know ‘BIGINTER (IMAGE A N) SUBSET p_space q’
         >- (fs [GSYM MEMBER_NOT_EMPTY, Abbr ‘A’, SUBSET_DEF] >> rw [] \\
             POP_ASSUM (STRIP_ASSUME_TAC o Q.SPEC ‘PREIMAGE ((X :'index -> 'b -> extreal) (x :'index))
                                                   ((E :'index -> extreal -> bool) (x :'index)) INTER p_space q’) \\
             Know ‘?n. PREIMAGE (X x) (E x) INTER p_space q =
                       PREIMAGE (X n) (E n) INTER p_space q /\ n IN N’
             >- (qexists ‘x’ >> fs []) >> DISCH_THEN (fs o wrap)) >> Rewr \\
         Know ‘BIGINTER (IMAGE A N) IN events q’
         >- (Know ‘!n. n IN N ==> A n IN events q’
             >- (rw [Abbr ‘A’, events_def, p_space_def] \\
                 irule MEASURABLE_PREIMAGE_BOREL \\
                 fs [SUBSET_DEF]) \\
             DISCH_TAC \\
             irule EVENTS_BIGINTER_FN >> fs [finite_countable] \\
             rw [IMAGE_DEF, SUBSET_DEF] \\
             Q.PAT_X_ASSUM ‘!n. n IN N ==> A n IN events q’ (STRIP_ASSUME_TAC o Q.SPEC ‘x'’) \\
             fs []) >> Rewr) \\
 ...
 >> fs [Abbr ‘A’, Abbr ‘B’]
 >> irule EXTREAL_PROD_IMAGE_EQ >> rw []
    >> MP_TAC (Q.SPECL [‘p’, ‘q’, ‘PREIMAGE (X x) ((E :'index -> extreal -> bool) x) INTER p_space q’] PROB_SND)
 >> simp []
 >> ‘PREIMAGE (X x) (E x) INTER p_space q IN events q’
...
Proof
 ...
 >> simp [] >> STRIP_TAC
 >> Q.ABBREV_TAC ‘(X' :num -> 'b # 'a list -> extreal) = λi. X i o FST’
 >> Q.ABBREV_TAC ‘Y' = λi. Y i o SND’
 ...
 >> Q.ABBREV_TAC ‘N1 = {i | i IN N /\ i < n}’
 >> Q.ABBREV_TAC ‘N2 = {i | i IN N /\ n <= i /\ i < 2 * n}’
 >> Know ‘N = N1 UNION N2’
 ...
 >> Q.ABBREV_TAC ‘A1 = BIGINTER (IMAGE (λi. PREIMAGE (λx. X i (FST x)) (E i) INTER p_space (p1 × p2)) N1)’
 >> Q.ABBREV_TAC ‘A2 = BIGINTER (IMAGE (λi. PREIMAGE (λx. Y (i - n) (SND x)) (E i) INTER p_space (p1 × p2)) N2)’
 >> Q.ABBREV_TAC ‘A3 = BIGINTER (IMAGE (λj. PREIMAGE (λx. Y j (SND x)) (E' j) INTER p_space (p1 × p2)) J)’
 >> Know ‘N = N1 UNION N2’
 >-(rw [Abbr ‘N1’, Abbr ‘N2’] >> rw [INTER_DEF, Once EXTENSION] \\
    EQ_TAC >> rw [] >> fs [SUBSET_DEF])
 >> DISCH_TAC
 ...
 >- (rw [Abbr ‘A2’, Abbr ‘A3’] >> AP_TERM_TAC \\
     ...
     qexists ‘i’ >> simp [Abbr ‘E'’])
 >> DISCH_TAC
 >> Know ‘BIGINTER (IMAGE (λi. PREIMAGE (λx. Z i x) (E i) INTER p_space (p1 × p2)) (N1 UNION N2)) = A1 INTER A2’
 >- (rw [Abbr ‘A1’, Abbr ‘A2’] \\
     Know ‘BIGINTER (IMAGE (λi. PREIMAGE (λx. Z i x) (E i) INTER p_space (p1 × p2)) N1) =
           BIGINTER (IMAGE (λi. PREIMAGE (λx. X i (FST x)) (E i) INTER p_space (p1 × p2)) N1)’
     >- (AP_TERM_TAC >> rw [Abbr ‘N1’, Abbr ‘Z’, o_DEF, Once EXTENSION, IMAGE_DEF]
         >> EQ_TAC >> rw [] >> qexists ‘i’ >> gs []) >> Rewr \\
     Know ‘BIGINTER (IMAGE (λi. PREIMAGE (λx. Z i x) (E i) INTER p_space (p1 × p2)) N2) =
           BIGINTER (IMAGE (λi. PREIMAGE (λx. Y (i - n) (SND x)) (E i) INTER p_space (p1 × p2)) N2)’
     >- (AP_TERM_TAC >> rw [Abbr ‘N2’, Abbr ‘Z’, o_DEF, Once EXTENSION, IMAGE_DEF] \\
         EQ_TAC >> rw [] >> qexists ‘i’ >> gs []) >> Rewr) >> Rewr
 >> Q.ABBREV_TAC ‘r = p1 CROSS p2’
 >> Q.ABBREV_TAC ‘e1 = BIGINTER (IMAGE (λi. PREIMAGE (X i) (E i) INTER p_space p1) N1)’
 >> Q.ABBREV_TAC ‘e2 = BIGINTER (IMAGE (λi. PREIMAGE (Y (i - n)) (E i) INTER p_space p2) N2)’
 >> Know ‘∏ (λi. prob r (PREIMAGE (λx. Y (i - n) (SND x)) (E i) INTER p_space r)) N2 =
          ∏ (λi. prob r (PREIMAGE (λx. Y i (SND x)) (E' i) INTER p_space r)) J’
 >- (rw [Abbr ‘E'’, Abbr ‘J’] \\
     ...
     >- (rw [INJ_DEF, Abbr ‘N2’, IMAGE_DEF, Once EXTENSION]
         >- (qexists ‘i’ >> fs []) \\
         qexists ‘i’ >> fs []) >> Rewr \\
     DISCH_THEN (MP_TAC o Q.SPEC ‘λi. prob (r :('b # 'a list) m_space)
                                           (PREIMAGE (λx. (Y :num -> 'a list -> extreal) i (SND x))
                                                     (E (i + n)) INTER p_space r)’) \\
 ...
 >- (fs [GSYM MEMBER_NOT_EMPTY, INTER_UNIV, Abbr ‘A1’] \\
     ...
     Know ‘BIGINTER (IMAGE (λi. PREIMAGE (λx. Z i x) (E i) INTER p_space r) N2) =
           BIGINTER (IMAGE (λi. PREIMAGE (λx. Y (i - n) (SND x)) (E i) INTER p_space r) N2)’
     >- (AP_TERM_TAC >> rw [IMAGE_DEF, Once EXTENSION] \\
         EQ_TAC >> rw [] >> qexists ‘i’
         >> (Know ‘PREIMAGE (λx. Z i x) (E i) INTER p_space r =
                   PREIMAGE (λx. Y (i - n) (SND x)) (E i) INTER p_space r’
             >- (fs [Abbr ‘Z’]) >> METIS_TAC [])) >> Rewr \\
     ‘∏ (λi. prob r (PREIMAGE (λx. Z i x) (E i) INTER p_space r)) N2 =
      ∏ (λi. prob r (PREIMAGE (λx. Y (i - n) (SND x)) (E i) INTER p_space r)) N2’
    ...
    (MP_TAC o (Q.SPECL [‘p1’, ‘p2’, ‘Y’, ‘J’]) o
     ...
     simp [] >> fs [] \\
     Know ‘!i. i IN J ==> Y i IN Borel_measurable (measurable_space p2)’
     >- (rw [Abbr ‘J’] \\
         ‘i' - n < n’ by (fs [SUBSET_DEF] \\
                          Q.PAT_X_ASSUM ‘!x'. x' IN N ==> x' < n \/ n <= x' /\ x' < 2 * n’
                           (STRIP_ASSUME_TAC o Q.SPEC ‘i'’) >> gs []) \\
     ...
     Suff ‘!i. i IN J ==> E' i IN subsets Borel’ >- (rw []) >> rw [Abbr ‘E'’] \\
     Q.PAT_X_ASSUM ‘!x. x IN N ==> E x IN subsets Borel’ (STRIP_ASSUME_TAC o Q.SPEC ‘i + n’) \\
     Know ‘i + n IN N’
     >- (fs [Abbr ‘J’] \\
 ...
 >- (fs [GSYM MEMBER_NOT_EMPTY, INTER_UNIV, Abbr ‘A2’] >> rw [Abbr ‘J’] \\
     ...
     Know ‘BIGINTER (IMAGE (λi. PREIMAGE (λx. Z i x) (E i) INTER p_space r) N) =
           BIGINTER (IMAGE (λi. PREIMAGE (λx. X i (FST x)) (E i) INTER p_space r) N)’
     >- (AP_TERM_TAC >> rw [IMAGE_DEF, Once EXTENSION] \\
         EQ_TAC >> rw [] >> qexists ‘i’
         >> (Know ‘PREIMAGE (λx. Z i x) (E i) INTER p_space r =
                   PREIMAGE (λx. X i (FST x)) (E i) INTER p_space r’
             >- (fs [Abbr ‘Z’]) >> METIS_TAC [])) >> Rewr \\
     ‘∏ (λi. prob r (PREIMAGE (λx. Z i x) (E i) INTER p_space r)) N =
      ∏ (λi. prob r (PREIMAGE (λx. X i (FST x)) (E i) INTER p_space r)) N’
       by (irule EXTREAL_PROD_IMAGE_EQ >> rw []) >> POP_ORW >> fs [] \\
     rpt (Q.PAT_X_ASSUM ‘T’ K_TAC) \\
     ...
     simp [] >> fs [] \\
     Know ‘!i. i IN N ==> X i IN Borel_measurable (measurable_space p1)’
 ...
 >> Know ‘∏ (λi. prob r (PREIMAGE (λx. Z i x) (E i) INTER p_space r)) (N1 UNION N2) =
          ∏ (λi. prob r (PREIMAGE (λx. Z i x) (E i) INTER p_space r)) N1 *
          ∏ (λi. prob r (PREIMAGE (λx. Z i x) (E i) INTER p_space r)) N2’
 >- (irule EXTREAL_PROD_IMAGE_DISJOINT_UNION >> fs [DISJOINT_DEF]) >> Rewr
 >> ‘!i x. i IN N1 ==> Z i x = X i (FST x)’ by fs [Abbr ‘Z’, Abbr ‘N1’]
 >> Know ‘BIGINTER (IMAGE (λi. PREIMAGE (λx. Z i x) (E i) INTER p_space r) N1) =
          BIGINTER (IMAGE (λi. PREIMAGE (λx. X i (FST x)) (E i) INTER p_space r) N1)’
 >- (AP_TERM_TAC >> rw [IMAGE_DEF, Once EXTENSION] \\
     EQ_TAC >> rw [] >> qexists ‘i’
     >> (Know ‘PREIMAGE (λx. Z i x) (E i) INTER p_space r =
               PREIMAGE (λx. X i (FST x)) (E i) INTER p_space r’
         >- (fs [Abbr ‘Z’]) >> METIS_TAC [])) >> Rewr
 >> ‘∏ (λi. prob r (PREIMAGE (λx. Z i x) (E i) INTER p_space r)) N1 =
     ∏ (λi. prob r (PREIMAGE (λx. X i (FST x)) (E i) INTER p_space r)) N1’
   by (irule EXTREAL_PROD_IMAGE_EQ >> rw []) >> POP_ORW >> fs []
 >> ‘!i x. i IN N2 ==> Z i x = Y (i - n) (SND x)’ by fs [Abbr ‘Z’, Abbr ‘N2’]
 >> Know ‘BIGINTER (IMAGE (λi. PREIMAGE (λx. Z i x) (E i) INTER p_space r) N2) =
          BIGINTER (IMAGE (λi. PREIMAGE (λx. Y (i - n) (SND x)) (E i) INTER p_space r) N2)’
 ...
 >> ‘∏ (λi. prob r (PREIMAGE (λx. Z i x) (E i) INTER p_space r)) N2 =
     ...
     by (irule EXTREAL_PROD_IMAGE_EQ >> rw []) >> POP_ORW >> fs []
 >> Q.PAT_X_ASSUM ‘T’ K_TAC
 >> Know ‘e1 IN events p1 /\ e2 IN events p2’
 >- (CONJ_TAC
     >- (rw [Abbr ‘e1’] >> fs [p_space_def, events_def, real_random_variable_def, random_variable_def] \\
         ...
         ‘N1 SUBSET (count n)’ by fs [SUBSET_DEF, Abbr ‘N1’] \\
            ‘!i. i IN N1 ==> X i IN Borel_measurable (measurable_space p1)’ by fs [SUBSET_DEF, count_def] \\
         METIS_TAC []) \\
 ...
 >- (rw [Abbr ‘A1’, Abbr ‘e1’, Abbr ‘r’, Once EXTENSION] \\
     ...
     >- (rw [IN_PREIMAGE, IN_INTER]
         (* FST x IN PREIMAGE (X i) (E i) INTER p_space p1 *)
         >- (Q.PAT_X_ASSUM ‘!P. (?i. P = PREIMAGE (λx. X i (FST x)) (E i) INTER p_space (p1 × p2) /\ i IN N1) ==> _’
              (STRIP_ASSUME_TAC o Q.SPEC ‘{x | (X :num -> 'b -> extreal) (i :num) (FST x) IN
                                               (E :num -> extreal -> bool) i} INTER p_space (p1 × p2)’) \\
             Know ‘?i'. {x | X i (FST x) IN E i} INTER p_space (p1 × p2) =
                             PREIMAGE (λx. X i' (FST x)) (E i') INTER p_space (p1 × p2) /\ i' IN N1’
     ...
     (* SND x IN p_space p2 *)
     >- (fs [GSYM MEMBER_NOT_EMPTY] >> rename1 ‘k IN N1’ \\
         Q.PAT_X_ASSUM ‘!P. (?i. P = PREIMAGE (λx. X i (FST x)) (E i) INTER p_space (p1 × p2) /\ i IN N1) ==> _’
          (STRIP_ASSUME_TAC o Q.SPEC ‘PREIMAGE (λx. (X :num -> 'b -> extreal) (k :num) (FST x))
                                      ((E :num -> extreal -> bool) k) INTER p_space (p1 × p2)’) \\
         Know ‘?i. PREIMAGE (λx. X k (FST x)) (E k) INTER p_space (p1 × p2) =
                   PREIMAGE (λx. X i (FST x)) (E i) INTER p_space (p1 × p2) /\ i IN N1’
         >- (fs [GSYM MEMBER_NOT_EMPTY] >>
             qexists ‘k’ >> METIS_TAC [PREIMAGE_def]) >> DISCH_THEN (fs o wrap) \\
         METIS_TAC [IN_PSPACE_PROD_SIGMA]) \\
     (* x IN PREIMAGE (λx. X i (FST x)) (E i) INTER p_space (p1 × p2) *)
     rw [PREIMAGE_def]
     (*  X i (FST x) IN E i *)
     >- (Q.PAT_X_ASSUM ‘!P. (?i. P = PREIMAGE (X i) (E i) INTER p_space p1 /\ i IN N1) ==> _’
          (STRIP_ASSUME_TAC o Q.SPEC ‘PREIMAGE ((X :num -> 'b -> extreal) (i :num) )
                                      ((E :num -> extreal -> bool) (i :num)) INTER p_space p1’)\\
         Know ‘?i'.
                 PREIMAGE (X i) (E i) INTER p_space p1 =
                 PREIMAGE (X i') (E i') INTER p_space p1 /\ i' IN N1’
 ...
 >- (rw [Abbr ‘A3’, Abbr ‘e2’, Abbr ‘r’, Once EXTENSION] \\
     EQ_TAC >> rw []
     >- (fs [GSYM MEMBER_NOT_EMPTY] >> rename1 ‘k IN J’ \\
         Q.PAT_X_ASSUM ‘!P. (?j. P = PREIMAGE (λx. Y j (SND x)) (E' j) INTER p_space (p1 × p2) /\ j IN J) ==> _’
          (STRIP_ASSUME_TAC o Q.SPEC ‘PREIMAGE (λx. (Y :num -> 'a list -> extreal) (k :num) (SND x))
                                      ((E' :num -> extreal -> bool) k) INTER p_space (p1 × p2)’) \\
         Know ‘?j. PREIMAGE (λx. Y k (SND x)) (E' k) INTER p_space (p1 × p2) =
                   PREIMAGE (λx. Y j (SND x)) (E' j) INTER p_space (p1 × p2) /\ j IN J’
         >- (fs [GSYM MEMBER_NOT_EMPTY] >>
             qexists ‘k’ >> METIS_TAC [PREIMAGE_def]) >> DISCH_THEN (fs o wrap) \\
         METIS_TAC [IN_PSPACE_PROD_SIGMA])
     >- (fs [Abbr ‘E'’] \\
         Q.PAT_X_ASSUM ‘!P. (?j. P = PREIMAGE (λx. Y j (SND x)) (E (j + n)) INTER p_space (p1 × p2) /\ j IN J) ==> _’
          (STRIP_ASSUME_TAC o Q.SPEC ‘PREIMAGE (λx. (Y :num -> 'a list -> extreal) (i - n) (SND x)) (E i) INTER
                                      p_space (p1 × p2)’) \\
         Know ‘?j. PREIMAGE (λx. Y (i - n) (SND x)) (E i) INTER p_space (p1 × p2) =
                   PREIMAGE (λx. Y j (SND x)) (E (j + n)) INTER p_space (p1 × p2) /\ j IN J’
         >- (qexists ‘i - n’ \\
     ...
     >- (‘n <= i’ by fs [Abbr ‘N2’] >> fs [Abbr ‘E'’, SUB_ADD] \\
         Q.PAT_X_ASSUM ‘!P. (?i. P = PREIMAGE (Y (i - n)) (E i) INTER p_space p2 /\ i IN N2) ==> _’
          (STRIP_ASSUME_TAC o Q.SPEC ‘PREIMAGE ((Y :num -> 'a list -> extreal) (i - n)) (E i) INTER
                                      p_space p2’)\\
         Know ‘?i'.
                 PREIMAGE (Y (i - n)) (E i) INTER p_space p2 =
                 PREIMAGE (Y (i' - n)) (E i') INTER p_space p2 /\ i' IN N2’
         >- (qexists ‘i’ >> rw []) >> DISCH_THEN (fs o wrap)) \\
     rw [IN_PSPACE_PROD_SIGMA] \\
     fs [GSYM MEMBER_NOT_EMPTY] >> rename1 ‘i IN N2’ \\
     Q.PAT_X_ASSUM ‘!P. (?i. P = PREIMAGE (Y (i - n)) (E i) INTER p_space p2 /\ i IN N2) ==> _’
      (STRIP_ASSUME_TAC o Q.SPEC ‘PREIMAGE (Y (i - n)) ((E :num -> extreal -> bool) i) INTER p_space p2’) \\
     Know ‘?i'.
             PREIMAGE (Y (i - n)) (E i) INTER p_space p2 =
             PREIMAGE (Y (i' - n)) (E i') INTER p_space p2 /\ i' IN N2’
     >- (qexists ‘i’ >> rw []) >> DISCH_THEN (fs o wrap)) >> Rewr
 >> Know ‘prob r (e1 × p_space p2 INTER p_space p1 × e2) = prob r (e1 CROSS e2)’
 ...
 >> Q.PAT_X_ASSUM ‘!e1 e2. e1 IN events p1 /\ e2 IN events p2 ==> _’ (STRIP_ASSUME_TAC o Q.SPECL [‘e1’, ‘e2’])
 >> gs []
 >> Know ‘∏ (λi. prob r (PREIMAGE (λx. Y i (SND x)) (E' i) INTER p_space r)) J =
          ∏ (λi. prob p2 (PREIMAGE (Y i) (E' i) INTER p_space p2)) J’
    >- (irule EXTREAL_PROD_IMAGE_EQ >> rw [Abbr ‘r’] \\
        (MP_TAC o (Q.SPECL [‘p1’, ‘p2’,
                            ‘PREIMAGE (Y (x :num)) ((E' :num -> extreal -> bool) x) INTER p_space p2’]) o
                (INST_TYPE [alpha |-> “:'b”, beta |-> “:('a list)”,  “:'index” |-> “:num”])) PROB_SND \\
        simp [] \\
        Know ‘PREIMAGE (Y x) (E' x) INTER p_space p2 IN events p2’
 ...
 >> Know ‘∏ (λi. prob r (PREIMAGE (λx. X i (FST x)) (E i) INTER p_space r)) N1 =
          ∏ (λi. prob p1 (PREIMAGE (X i) (E i) INTER p_space p1)) N1’
 >- (irule EXTREAL_PROD_IMAGE_EQ >> rw [Abbr ‘r’] \\
     (MP_TAC o (Q.SPECL [‘p1’, ‘p2’, ‘PREIMAGE (X (x :num)) ((E :num -> extreal -> bool) x) INTER p_space p1’]) o
             (INST_TYPE [beta |-> “:('a list)”, alpha |-> “:'b”, “:'index” |-> “:num”])) PROB_FST \\
     simp [] \\
     Know ‘PREIMAGE (X x) (E x) INTER p_space p1 IN events p1’
     >- (rw [Abbr ‘E'’, p_space_def, events_def] \\
         irule MEASURABLE_PREIMAGE_BOREL \\
         fs [] \\
     ...
     AP_TERM_TAC >> rw [PREIMAGE_def, INTER_DEF, Once EXTENSION] \\
     EQ_TAC >> rw [] >> METIS_TAC [IN_PSPACE_PROD_SIGMA]) >> Rewr
    >> Know ‘prob p1 e1 = ∏ (λi. prob p1 (PREIMAGE (X i) (E i) INTER p_space p1)) N1’
 >- (simp [Abbr ‘e1’] >> irule PROB_BIGINTER_INDEP >> fs [] \\
     ‘N1 SUBSET (count n)’ by fs [SUBSET_DEF, Abbr ‘N1’] \\
     METIS_TAC [indep_vars_subset]) >> Rewr
 >> Know ‘prob p2 e2 = ∏ (λi. prob p2 (PREIMAGE (Y i) (E' i) INTER p_space p2)) J’
 >- (simp [Abbr ‘e2’] \\
     Know ‘BIGINTER (IMAGE (λi. PREIMAGE (Y (i - n)) (E i) INTER p_space p2) N2) =
           BIGINTER (IMAGE (λj. PREIMAGE (Y j) (E (j + n)) INTER p_space p2) J)’
     ...
     >- (Know ‘J SUBSET (count n)’
         >- (rw [SUBSET_DEF, Abbr ‘J’, Abbr ‘N2’, IMAGE_DEF]
             >> (NTAC 2 (POP_ASSUM MP_TAC) >> numLib.ARITH_TAC)) >> DISCH_TAC \\
         METIS_TAC [indep_vars_subset]) >> Rewr \\
     Know ‘!i. i IN J ==> E (i + n) IN subsets Borel’
     >- (rw [] >> Q.PAT_X_ASSUM ‘!x. x IN N1 \/ x IN N2 ==> _’ (STRIP_ASSUME_TAC o Q.SPEC ‘i + n’) \\
...
Theorem indep_vars_Borel_imp_borel :
    !p X Y. prob_space p /\
            (!x. x IN p_space p ==> X x <> PosInf /\ X x <> NegInf) /\
            (!x. x IN p_space p ==> Y x <> PosInf /\ Y x <> NegInf) /\
            indep_rv p X Y Borel Borel ==>
            indep_rv p (real o X) (real o Y) borel borel
Proof
    rpt STRIP_TAC
 ...
 >> STRIP_TAC
 >> ‘IMAGE Normal a IN subsets Borel /\ IMAGE Normal b IN subsets Borel’ by
   METIS_TAC [BOREL_MEASURABLE_SETS_NORMAL] >> gs []
 >> Know ‘PREIMAGE X (IMAGE Normal a) INTER m_space p = PREIMAGE (λx. real (X x)) a INTER m_space p’
 ...
 >> Know ‘PREIMAGE Y (IMAGE Normal b) INTER m_space p = PREIMAGE (λx. real (Y x)) b INTER m_space p’
 >- (rw [PREIMAGE_def, INTER_DEF, IMAGE_DEF, Once EXTENSION] \\
     EQ_TAC >> rw [] >- (fs [real_normal]) \\
...
Proof
 ...
 >> ‘indep_vars p (real o X) (real o Y) borel borel’ by METIS_TAC [indep_vars_Borel_imp_borel]
 >> POP_ORW >> rw [o_DEF]
...
Theorem sum_list_eq_EXTREAL_SUM_IMAGE :
    !f s. FINITE s /\ (!x. x IN s ==> f x <> PosInf /\ f x <> NegInf) ==>
          ∑ f s = sum_list (MAP f (SET_TO_LIST s))
Proof
    rpt STRIP_TAC
 >> Q.ABBREV_TAC ‘l = MAP f (SET_TO_LIST s)’
 >> Know ‘!x. MEM x l ==> x <> PosInf /\ x <> NegInf’
 ...
 >> rw []
 >- (Q.PAT_X_ASSUM ‘!x'. x' IN x INSERT t ==> _’ (STRIP_ASSUME_TAC o Q.SPEC ‘x’)\\
     gs [COMPONENT] \\
...
Proof
 ...
 >- (Q.PAT_X_ASSUM ‘!a b. _’ (STRIP_ASSUME_TAC o Q.SPECL [‘a’, ‘b’]) \\
     ...
     ‘{x | f x IN a /\ x IN m_space p} = {x | h x IN a /\ x IN m_space p}’ by ASM_SET_TAC [] \\
     gs [])
 >- (Q.PAT_X_ASSUM ‘!a b. _’ (STRIP_ASSUME_TAC o Q.SPECL [‘a’, ‘b’]) \\
     gs [PREIMAGE_def, INTER_DEF, p_space_def, events_def])
 >> Q.PAT_X_ASSUM ‘!a b. _’ (STRIP_ASSUME_TAC o Q.SPECL [‘a’, ‘b’])
 >> gs [PREIMAGE_def, INTER_DEF, p_space_def, events_def]
 >> ‘{x | f x IN a /\ x IN m_space p} = {x | h x IN a /\ x IN m_space p}’ by ASM_SET_TAC []
 >> ‘{x | (h x IN a /\ x IN m_space p) /\ g x IN b /\ x IN m_space p} =
     {x | (f x IN a /\ x IN m_space p) /\ g x IN b /\ x IN m_space p}’ by ASM_SET_TAC []
 >> gs []
QED
...
Theorem EXTREAL_SUM_IMAGE_EQ_SUM_LIST :
    !n f. (!i. i < n ==> f i <> PosInf /\ f i <> NegInf) ==> ∑ (λi. f i) (count n) = sum_list (GENLIST f n)
Proof
    Induct_on ‘n’ >> rw []
 >> ‘count1 n = (count n) UNION {n}’
   by fs [count1_def, count_def, UNION_DEF, Once EXTENSION] >> gs [ETA_AX]
 >> Know ‘∑ f (count n UNION {n}) = ∑ f (count n) + f n’
...
Proof
  ...
  >> Q.ABBREV_TAC ‘A = pos_fn_integral p (λx. abs (X x) powr 2)’
  >> Q.ABBREV_TAC ‘B = pos_fn_integral p (λx. abs (X x) powr 3)’
...
Theorem EVENT_INTER_SUBR :
    !p A. prob_space p /\ A SUBSET p_space p ==> A INTER p_space p = A
Proof
...
Theorem PREIMAGE_FST :
  !p q f A.
    prob_space p /\ prob_space q ==>
    (!s. s IN subsets A ==>
         PREIMAGE (f o FST) s INTER p_space (p × q) =
         (PREIMAGE f s × p_space q) INTER (p_space p × p_space q))
Proof
...
Theorem PREIMAGE_SND :
    !p q f A.
      prob_space p /\ prob_space q ==>
      (!s. s IN subsets A ==>
           PREIMAGE (f o SND) s INTER p_space (p × q) =
           (p_space p CROSS PREIMAGE f s) INTER (p_space p × p_space q))
Proof
    rw [PREIMAGE_def, Once EXTENSION]
 >> EQ_TAC >> rw [IN_PSPACE_PROD_SIGMA]
QED
...
Theorem MEASURABLE_PREIMAGE :
    !f m A.
      f IN measurable (measurable_space m) A ==>
      !s. s IN subsets A ==>
          PREIMAGE f s INTER m_space m IN measurable_sets m
Proof
    rpt STRIP_TAC
...
Theorem PROB_FST' :
    !p q X A.
      prob_space p /\ prob_space q /\
      X IN measurable (measurable_space p) A ==>
      (!s. s IN subsets A ==>
           prob (p × q) (PREIMAGE (X o FST) s INTER p_space (p × q)) =
...
Proof
 ...
 >> rw [prod_measure_space_def, prod_sigma_def, SPACE_PROD_SIGMA]
 >> MP_TAC (Q.SPECL [‘p’, ‘q’, ‘PREIMAGE (X :'a -> 'c) s INTER m_space p’, ‘m_space q’] PROD_MEASURE_CROSS)
 >> impl_tac
...
Theorem PROB_SND' :
    !p q X A.
      prob_space p /\ prob_space q /\
      X IN measurable (measurable_space q) A ==>
      (!s. s IN subsets A ==>
           prob (p × q) (PREIMAGE (X o SND) s INTER p_space (p × q)) =
...
Proof
 ...
 >> MP_TAC (Q.SPECL [‘p’, ‘q’, ‘m_space p’,
                     ‘m_space q INTER PREIMAGE (X :'b -> 'c) s’] PROD_MEASURE_CROSS)
 >> impl_tac
...
Theorem DISTRIBUTION_FST :
    !p q X A.
      ...
      X IN measurable (measurable_space p) A ==>
      (!s. s IN subsets A ==>
           distribution (p × q) (X o FST) s = distribution p X s)
Proof
...
Theorem DISTRIBUTION_SND :
    !p q X A.
      ...
      X IN measurable (measurable_space q) A ==>
      (!s. s IN subsets A ==>
           distribution (p × q) (X o SND) s = distribution q X s)
Proof
...
Theorem ext_normal_rv_fst :
    !p q X mu sig.
      ...
      ext_normal_rv (X o FST) (p × q) mu sig
Proof
    rpt STRIP_TAC
 >> Know ‘(p_space (p × q),events (p × q)) =
          (p_space p,events p) × (p_space q,events q)’
 ...
 >> Q.PAT_X_ASSUM ‘!s. s IN subsets borel ==> _’ (STRIP_ASSUME_TAC o Q.SPEC ‘s’) >> gs []
 >> POP_ASSUM (rw o wrap o SYM)
 >> MP_TAC (Q.SPECL [‘p’, ‘q’, ‘real o X’, ‘borel’] (INST_TYPE [“:'c” |-> “:real”] DISTRIBUTION_FST))
 >> fs [p_space_def, events_def]
QED
...
Theorem ext_normal_rv_snd :
    !p q X mu sig.
      ...
      ext_normal_rv (X o SND) (p × q) mu sig
Proof
    rpt STRIP_TAC
 >> Know ‘(p_space (p × q),events (p × q)) =
          (p_space p,events p) × (p_space q,events q)’
 ...
 >> Q.PAT_X_ASSUM ‘!s. s IN subsets borel ==> _’ (STRIP_ASSUME_TAC o Q.SPEC ‘s’) >> gs []
 >> POP_ASSUM (rw o wrap o SYM)
 >> MP_TAC (Q.SPECL [‘p’, ‘q’, ‘real o X’, ‘borel’] (INST_TYPE [“:'c” |-> “:real”] DISTRIBUTION_SND))
 >> fs [p_space_def, events_def]
QED
...
Theorem expectation_from_distribution_eq :
    !p q X Y f.
      ...
      random_variable Y q Borel /\ f IN Borel_measurable Borel /\
      (!s. s IN subsets Borel ==> distribution p X s = distribution q Y s) ==>
      expectation p (f o X) = expectation q (f o Y)
Proof
    rpt STRIP_TAC
...
Theorem distribution_real_eq :
    !p X.
    prob_space p /\ real_random_variable X p ==>
    (!s. s IN subsets Borel ==> distribution p (real o X) (real_set s) = distribution p X s)
Proof
...
Theorem IN_MEASURABLE_BOREL_FST :
    !a b f.
      sigma_algebra a /\ sigma_algebra b /\ f IN Borel_measurable a ==>
      f o FST IN Borel_measurable (a CROSS b)
Proof
    rpt STRIP_TAC
 >> MATCH_MP_TAC MEASURABLE_COMP
 >> qexists ‘a’
...
Theorem IN_MEASURABLE_BOREL_SND :
    !a b f.
      sigma_algebra a /\ sigma_algebra b /\ f IN Borel_measurable b ==>
      f o SND IN Borel_measurable (a CROSS b)
Proof
    rpt STRIP_TAC
 >> MATCH_MP_TAC MEASURABLE_COMP
 >> qexists ‘b’
...
Theorem distribution_eq_comp :
    !p q X Y f.
      ...
      (!s. s IN subsets borel ==> distribution p X s = distribution q Y s) /\
      f IN borel_measurable borel ==>
      (!s. s IN subsets borel ==> distribution p (f o X) s = distribution q (f o Y) s)
Proof
    rw [distribution_def, GSYM PREIMAGE_COMP]
 >> FIRST_X_ASSUM (STRIP_ASSUME_TAC o Q.SPEC ‘PREIMAGE f (s :real -> bool)’)
 >> Suff ‘PREIMAGE f s IN subsets borel’ >> rw []
 >> POP_ASSUM K_TAC
...
Theorem variance_fst :
    !p q f.
      ...
      variance p f = variance (p × q) (f o FST)
Proof
    rpt STRIP_TAC
 ...
 >> impl_tac >- (fs [PROD_PROB_SPACE] \\
                 ...
                 fs [o_DEF])
 >> Rewr
 >> ‘expectation p f = expectation (p × q) (f o FST)’ by METIS_TAC [expectation_fst]
 >> POP_ORW
 >> Suff ‘expectation p (λx. (f x)²) =  expectation (p × q) (λx. ((f o FST) x)²)’
 >> fs []
...
Theorem variance_snd :
    !p q f.
      ...
      variance q f = variance (p × q) (f o SND)
Proof
    rpt STRIP_TAC
 ...
 >> impl_tac >- (fs [PROD_PROB_SPACE] \\
                 ...
                 fs [o_DEF])
 >> Rewr
 >> ‘expectation q f = expectation (p × q) (f o SND)’ by METIS_TAC [expectation_snd]
 >> POP_ORW
 >> Suff ‘expectation q (λx. (f x)²) =  expectation (p × q) (λx. ((f o SND) x)²)’
 >> fs []
...
Theorem real_lt_eq :
    !x y. x <> PosInf /\ x <> NegInf /\ y <> PosInf /\ y <> NegInf ==> (real x < real y <=> x < y)
Proof
    rpt STRIP_TAC
...
Proof
  ...
  >> rw [indep_rv_def, indep_def]
  >> fs [random_variable_def, p_space_def, events_def]
  >> ‘PREIMAGE (λx. c) b INTER m_space p = (if c IN b then m_space p else {} )’
...
val bn_not_infty_tactic =
    ...
    (* ∑ (λi. expectation p (λx. (abs (X i x))³)) (count z) <> PosInf *)
...
val clt_tactic3_p1 =
    ...
    >- (rw [Abbr ‘sig’] \\
        ...
        Q.PAT_X_ASSUM ‘!i. 0 <= variance p (X i)’ (STRIP_ASSUME_TAC o Q.SPEC ‘i’) \\
        Q.PAT_X_ASSUM ‘!i. variance p (X i) <> NegInf /\ _’ (STRIP_ASSUME_TAC o Q.SPEC ‘i’) \\
...
val clt_tactic3_p3 =
    Q.ABBREV_TAC ‘(Y' :num -> 'a # 'a list -> extreal) = λi. Y i o SND’ >> fs []
 >> Q.ABBREV_TAC ‘(X' :num -> 'a # 'a list -> extreal) = λi. X i o FST’ >> fs []
 >> ‘!x. (R n o FST) x = ∑ (λi. X' i x) (count (SUC n)) / s (SUC n)’ by rw [Abbr ‘X'’, Abbr ‘R’]
 >> Q.ABBREV_TAC ‘r = p CROSS p'’
 >> Know ‘expectation p (Normal o f o real o R n) =
          expectation r (Normal o f o real o R n o FST)’
 >- (Q.ABBREV_TAC ‘h = Normal o f o real o R n’ \\
     ‘expectation r (Normal o f o real o R n o FST) = expectation r (h o FST)’ by rw [Abbr ‘h’] \\
 ...
 >> Q.ABBREV_TAC ‘g = R n o (FST :'a # 'a list -> 'a)’
 >> Q.ABBREV_TAC ‘h = λx. ∑ (λi. X' i x) (count (SUC n)) / s (SUC n)’
 >> (MP_TAC o (Q.SPECL [‘r’, ‘g’, ‘h’]) o
            (INST_TYPE [alpha |-> ``: ('a # 'a list)``])) expectation_cong  >> rw []
 >> ‘expectation r (Normal o f o real o g) = expectation r (Normal o f o real o h)’
   by METIS_TAC [expectation_cong] >> POP_ORW
 >> rw [Abbr ‘h’]
 >> Know ‘?c. s (SUC n) = Normal c’
 >- (Q.PAT_X_ASSUM ‘!n. s n <> NegInf /\ s n <> PosInf’ (STRIP_ASSUME_TAC o Q.SPEC ‘SUC n’) \\
...
val clt_tactic3_p4 =
    Know ‘expectation p (Normal o f o real o N) =
          expectation r (Normal o f o real o (λx. ∑ (λi. Y' i x) (count (SUC n)) / Normal c))’
 >- (rw [Abbr ‘Y'’] \\
     ...
     >- (rw [] \\
         ...
         rw [Abbr ‘r’]) \\
     DISCH_TAC \\
     MP_TAC (Q.SPECL [‘r’, ‘λi. Y i o SND’, ‘λi. 0’, ‘(λi. sig (i :num))’, ‘SUC n’]
     ...
     qmatch_abbrev_tac ‘expectation p (Normal o f o real o N) =
                        expectation r (Normal o f o real o G)’ \\
     ‘G  = (λx. ∑ (λi. Y i x) (count (SUC n)) / Normal c) o SND’ by rw [Abbr ‘G’, o_DEF] \\
     POP_ASSUM (fs o wrap) >> rw [Abbr ‘G’] \\
     qmatch_abbrev_tac ‘expectation p (Normal o f o real o N) =
                        expectation r (Normal o f o real o G o SND)’ \\
     ...
     DISCH_TAC \\
     Know ‘expectation r (Normal o f o real o G o SND) =
           expectation p' (Normal o f o real o G)’
     >- (rw [Abbr ‘r’, expectation_def] \\
         Q.ABBREV_TAC ‘h = Normal o f o real o G’ \\
         MP_TAC (Q.SPECL [‘p’, ‘p'’, ‘h o SND’]
         ...
         >- (‘sigma_finite_measure_space p /\ sigma_finite_measure_space p'’
             ...
             Know ‘∫ (p × p') (abs o h o SND) <> PosInf’
             >- (qmatch_abbrev_tac ‘∫ (p × p') (abs o H) <> PosInf’ \\
         ...
         Q.PAT_X_ASSUM ‘!x'. x' IN m_space p' ==> G x' <> NegInf /\ G x' <> PosInf’ (STRIP_ASSUME_TAC o Q.SPEC ‘x’)
         >> gs [p_space_def]
     ...
     rw [real_random_variable_def, random_variable_borel_imp_Borel] \\
     POP_ASSUM (STRIP_ASSUME_TAC o Q.SPEC ‘t’) >> gs [] \\
     Q.PAT_X_ASSUM ‘!s. s IN subsets borel ==> distribution p' (real o G) s = _’
...
val clt_tactic3_p5 =
 ...
 >- (simp [] >> GEN_TAC >> STRIP_TAC \\
     ...
     >- (MP_TAC (Q.SPECL [‘r’, ‘λx. (X' (j :num) x)’]
                  (INST_TYPE [“:'a” |-> “:('a # 'a list)”] variance_alt)) >> rw [] \\
     ...
     ‘!x. x IN p_space r ==> Z j x = Z0 j x’ by rw [Abbr ‘Z’, Abbr ‘Z0’] \\
     ‘!A. PREIMAGE (Z j) A INTER p_space r = PREIMAGE (Z0 j) A INTER p_space r’
       by (rw [PREIMAGE_def, INTER_DEF, Once EXTENSION] \\
           EQ_TAC >> rw [] >> METIS_TAC []) \\
     POP_ORW \\
     ...
     Q.ABBREV_TAC ‘t = (count (SUC n) DIFF count1 j) UNION IMAGE (λi. (SUC n) + i) (count j)’ \\
     ...
     >- (rw [] \\
         MP_TAC (Q.SPECL [‘λ(n :num). W0 n (x : 'a # 'a list)’, ‘t’]
         ...
         rw [Abbr ‘t’] \\
         Know ‘∑ (λn. W0 n x) (count (SUC n) DIFF count1 j UNION IMAGE (λi. i + (SUC n)) (count j)) =
     ...
     >- (simp [indep_rv_def, indep_def] \\
         ...
         (* PREIMAGE (X' j) a1 INTER p_space r IN events r *)
         STRONG_CONJ_TAC
         ...
         MP_TAC (Q.SPECL [‘r’, ‘W0’, ‘λi. Borel’, ‘u UNION t’]
                  (INST_TYPE [“:'a” |-> “:('a # 'a list)”, “:'b” |-> “:extreal”,
         ...
         impl_tac >- (ASM_SIMP_TAC std_ss [] \\
                      ...
                      >- (rw [MEM_APPEND, Abbr ‘W0’, Abbr ‘X'’, Abbr ‘Y'’] \\
                          ...
                          Q.PAT_X_ASSUM ‘!i. i < (SUC n) ==>
                                             (λx. Y i (SND x)) IN Borel_measurable _ /\ !x. x IN m_space r ==> _’
         ...
         Q.PAT_X_ASSUM ‘!a b. a IN subsets Borel /\ b IN subsets Borel ==> _’
          (STRIP_ASSUME_TAC o Q.SPECL [‘a1’, ‘a2’]) >> gs [] \\
         ‘!x. PREIMAGE (λx. W0 j x) a1 INTER p_space r = PREIMAGE (X' j) a1 INTER p_space r’
           by (rw [PREIMAGE_def, INTER_DEF, Once EXTENSION] \\
               EQ_TAC >> rw [] >> METIS_TAC []) \\
         ‘!x. PREIMAGE (λx. sum_list (MAP (λn. W0 n x) (SET_TO_LIST t))) a2 INTER p_space r =
              PREIMAGE (Z0 j) a2 INTER p_space r’
           by (rw [PREIMAGE_def, INTER_DEF, Once EXTENSION] \\
               EQ_TAC >> rw [] >> METIS_TAC []) \\
         ‘!x. PREIMAGE (λx. W0 j x) a1 INTER p_space r INTER
                       (PREIMAGE (λx. sum_list (MAP (λn. W0 n x) (SET_TO_LIST t))) a2 INTER p_space r) =
              PREIMAGE (X' j) a1 INTER p_space r INTER (PREIMAGE (Z0 j) a2 INTER p_space r)’
           by (rw [PREIMAGE_def, INTER_DEF, Once EXTENSION, UNION_DEF] \\
               EQ_TAC >> rw [] >> METIS_TAC []) \\
         METIS_TAC []) \\
     simp [indep_rv_def, indep_def] \\
     Q.X_GEN_TAC ‘a1’ >> Q.X_GEN_TAC ‘a2’ >> STRIP_TAC \\
     (* PREIMAGE (Y' j) a1 INTER p_space r IN events r *)
     STRONG_CONJ_TAC
     >- (rw [p_space_def, events_def] \\
         irule MEASURABLE_PREIMAGE \\
         qexists ‘Borel’ >> fs [real_random_variable, p_space_def, Abbr ‘X'’, Abbr ‘r’] \\
     ...
     Q.PAT_X_ASSUM ‘!a b. a IN subsets Borel /\ b IN subsets Borel ==> _’
      (STRIP_ASSUME_TAC o Q.SPECL [‘a1’, ‘a2’]) >> gs [] \\
     ‘!x. PREIMAGE (λx. W0 (j + (SUC n)) x) a1 INTER p_space r = PREIMAGE (Y' j) a1 INTER p_space r’
       by (rw [PREIMAGE_def, INTER_DEF, Once EXTENSION] \\
           EQ_TAC >> rw [] >> METIS_TAC []) \\
     ‘!x. PREIMAGE (λx. sum_list (MAP (λn. W0 n x) (SET_TO_LIST t))) a2 INTER p_space r =
          PREIMAGE (Z0 j) a2 INTER p_space r’
       by (rw [PREIMAGE_def, INTER_DEF, Once EXTENSION] \\
           EQ_TAC >> rw [] >> METIS_TAC []) \\
     ‘!x. PREIMAGE (λx. W0 (j + (SUC n)) x) a1 INTER p_space r INTER
                   (PREIMAGE (λx. sum_list (MAP (λn. W0 n x) (SET_TO_LIST t))) a2 INTER p_space r) =
          PREIMAGE (Y' j) a1 INTER p_space r INTER (PREIMAGE (Z0 j) a2 INTER p_space r)’
       by (rw [PREIMAGE_def, INTER_DEF, Once EXTENSION, UNION_DEF] \\
           EQ_TAC >> rw [] >> METIS_TAC []) \\
     METIS_TAC [])
 >> DISCH_TAC >> gs [];
...
Theorem clt_expectation_sum_not_infty1[local] :
  !p X s R f n.
    ...
    (!i. variance p (X i) < PosInf) /\
    (!i. expectation p (X i) = 0) /\
    0 < s (SUC n) /\ s (SUC n) <> PosInf /\ s (SUC n) <> NegInf /\
...
Proof
    ...
    >> Q.X_GEN_TAC ‘i’ >> STRIP_TAC
    >> MP_TAC (Q.SPECL [‘p’, ‘λx'. (abs (X (x :num) x'))³’, ‘8’] (GSYM expectation_cmul))
    >> rw []
